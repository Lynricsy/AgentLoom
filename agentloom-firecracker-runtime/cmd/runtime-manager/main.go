...
func main() {
	...
	check, err := preflight.Check(preflight.Config{
		...
		AllowUnsupportedKernel: config.AllowUnsupported, AllowSMT: config.AllowSMT, SkipDeviceChecks: config.SkipDeviceChecks,
	})
	if err != nil {
	...
	firewall, err := networkpkg.NewNFTables("nft", 18080)
	...
	provisioner, err := networkpkg.NewCNIProvisioner(networkpkg.Config{
		...
		Gateway: config.GuestGateway, Nameservers: []string{"1.1.1.1", "1.0.0.1"}, TapName: "tap0", Firewall: firewall,
	})
	fatalIf(logger, err, "initialize CNI")
	launcher, err := runtimepkg.NewFirecrackerLauncher(runtimepkg.LauncherConfig{
		ChrootBase: filepath.Join(config.StateRoot, "jailer"), JailerWrapper: config.JailerWrapper,
	...
	guestTLS, err := clientTLSConfig(config.GuestCA, config.GuestServerName)

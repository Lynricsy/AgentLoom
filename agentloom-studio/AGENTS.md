...
| `/generated-apps/public/$token`                  | GeneratedAppPublicRuntimePage  | Generated App 公开 runtime 入口：无需登录，只展示 title、description、dataUseNotice、有限 AppSpec、runtime preview link 和公开 submission 提交/结果，不渲染 Studio 壳层                                                           |
...
│   ├── generated-app/ # 一句话生成应用 Studio 工作台与公开 runtime：creator API/hooks 管理 readiness/public share/submissions/generation runs/repair attempts/gate runs/start runner；public API helpers 与内置公开页可提交/读取 submission，公开页只渲染 end-user runtime/submission 白名单字段

...
- **执行 API 层** (`features/execution/api/`):
  ...
  - **Story 8-6 输入参数启动流**: `runWorkflow()` 现接受 `{ inputParams?, schemaVersion?, launchSource? }`；`useStartExecution()` 同步透传这些字段并继续桥接 `executionStore.initExecution(id)`；Web Studio 启动固定使用 `launchSource: 'web-studio'`
...
- **Trigger feature（Story 8-3 审查修复）**: `features/trigger/` 现要求普通 webhook config 只保证 `token + ipWhitelist`，`secret` 仅在创建成功返回中可选出现；`TriggerHistoryStatus` 已扩展 `signature_failed`；`WebhookConfigForm` 编辑态只展示 URL/Token 与“一次性 secret”提示，不再重复展示 secret；`API Event` 已收口为真正 preview-only：dialog 提交按钮禁用、表单只读、card 不允许编辑/启停，server 也会对 create/update/toggle 返回 409；当前 workflow 详情页仍然是 `WorkflowCanvasPage`（`/workflows/$workflowId`），不要为触发器发明独立 detail route
...
- **Story 8-6 工作流输入参数**: `features/workflow/` 现定义 canonical `WorkflowInputSchema` / `WorkflowInputFieldDefinition` / `WorkflowInputFieldVisibility` 等类型，`useWorkflowInputSchema(workflowId, { enabled? })` 对接发布态 `GET /workflow-definitions/:id/input-schema`；`features/workflow-input-schema/` 提供 `InputSchemaRenderer`（共享字段渲染层）、`WorkflowInputSchemaTab`（`collectionMode='form'` 的编辑器，支持字段上下重排、同源 live preview）与 `ExecutionLaunchDialog`（visibility 感知字段渲染、动态校验、提交 `inputParams + schemaVersion + launchSource='web-studio'`）。`WorkflowCanvasPage + WorkflowSettingsPanel + VersionToolbar` 继续沿用左上共享 settings panel 模式接入“输入参数”tab 与运行弹窗：草稿态编辑走 detail `inputSchema` + `PATCH /workflow-definitions/:id`（只需 `{ version, inputSchema }`），发布态启动按需查询 `/input-schema`，触发器/介入策略 tab 仍用 `hidden + data-active` 常驻内容区避免卸载本地状态；viewer/operator 在 `WorkflowInputSchemaTab` 中只读，Run 入口仅对 `workflow.status === 'published'` 暴露，避免把 draft 工作流带到最终只能 409 的后端路径。

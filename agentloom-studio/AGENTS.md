...
- **Marketplace feature** (`features/marketplace/`): 同时覆盖发布者后台与公共浏览链路。公共侧数据层使用 `publicMarketplaceKeys/publicMarketplaceApi/publicMarketplaceQueries/publicMarketplaceMutations`，对应 `/marketplace/browse`（公开列表/详情/评论）与已认证 `/marketplace/listings/:id/install|reviews`；页面层形成 `MarketplaceBrowsePage` → `MarketplaceDetailDialog` → `MarketplaceInstallDialog/ReviewForm` 闭环，浏览页顶部保留 `/marketplace/my-listings` 入口回到发布管理

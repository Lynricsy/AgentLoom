...
| `/generated-apps/$appId`                         | GeneratedAppDetailPage         | Generated App 创建者详情页：AppSpec、验收场景、Gate 结果、Traceability、Artifacts、受控 workspace 源码/测试/报告预览、Gate 3 构建产物沙箱预览、Resource bindings、自动生成与验证运行、公开链接管理、公开提交记录管理与生成证据/运行记录查看；生成证据区默认选中最新可见 generation run，并显示修复尝试的结构化修复工作单与再验证计划摘要 |
| `/generated-apps/public/$token`                  | GeneratedAppPublicRuntimePage  | Generated App 公开 runtime 入口：无需登录，只展示 title、description、dataUseNotice、有限 AppSpec、动态业务表单、runtime preview link 和结构化 submission 报告/结果，不渲染 Studio 壳层；公开页会渲染所有白名单 `runtimeForm.fields[]`，未被任何 section 引用的字段进入本地兜底分组，避免必填字段不可见；当后端返回 `/api/v1/generated-apps/public/:token/preview` 时，API mapping 会在绝对 `VITE_API_BASE_URL` 部署下把预览链接解析到 API 域名，公开页只提供打开链接，不调用 creator artifact 接口、不渲染源码/测试 artifact 或 workspace metadata；被打开的 public build preview HTML 自身只可调用同源 public submission API |
...
| `/agents/$agentId/conversations/new`             | NewConversationDraftPage       | 新对话草稿态页；首条消息发送成功后才创建真实 conversation 并跳转正式对话页；`sandbox` 草稿态展示持久化工作区预览，`no_sandbox` 草稿态不预留右侧上下文占位列                                                                       |
| `/agents/$agentId/conversations/$conversationId` | AgentConversationPage          | 三列对话 UI (对话列表/消息流/上下文面板)；按 Agent `runtimeMode` 显示 `有沙箱 / 无沙箱` 标记，`no_sandbox` 不渲染右侧工作区/进程上下文列，仅保留消息流；输入栏支持图片/文件草稿队列，多附件与文本同发，用户消息可回显全部附件预览 |
...
│   ├── generated-app/ # 一句话生成应用 Studio 工作台与公开 runtime：creator API/hooks 管理 readiness/public share/submissions/generation runs/repair attempts/gate runs/start runner/受控 workspace artifact manifest + content；生成证据面板展示 repair attempts、结构化修复工作单、再验证计划和 Gate evidence；public API helpers 与内置公开页可提交/读取 submission，公开页只渲染 end-user runtime/submission 白名单字段、动态业务表单、未分组字段兜底分组、可选 public build preview link 和结构化报告视图；public build preview HTML 只在打开后走同源 public submission API
...
- **Agent 对话 Socket.IO**: `/agent-conversation` namespace，与 `/execution` 对称，复用 EventBridge 模式实现对话级实时事件推送，支持 JWT + MFA 认证；`useAgentConversationSocket` 管理连接与事件监听，桥接 `agentConversationStore` 更新消息流；`agentConversationStore.actions.sendMessage()` 支持 `string | OutgoingConversationMessage`，会保留 `contentType + metadata.attachments[]`（并兼容 legacy `metadata.attachment`）做 optimistic user message、socket emit 与 history round-trip；`AgentConversationPage` 输入栏会先把图片/文件放进输入栏上方草稿区，再由用户点击发送一次性提交同一条文本 + 多附件消息；单附件上限 `1.5 MB`、单消息附件总量上限 `10 MB`、文本内联上限 `200 KB`，文本文件优先以内联文本资源进入上下文，二进制文件与图片走 `dataBase64`，sandbox runtime 会在 prompt 中额外提示 `/workspace/uploads/...` 路径；`MessageList` 与共享 `ToolCallCard` 负责渲染自进化 diff/风险信息、四档审批按钮（允许一次 / 本会话同类始终允许 / 拒绝一次 / 本会话同类始终拒绝）、“刷新当前对话”卡片，以及用户侧全部图片预览/文件卡片/文本文件内容预览；`PreparationCard` / `AgentConversationPage` 会根据 `runtimeMode` 区分“沙箱启动中”与“无沙箱 Agent 无需准备沙箱”，并避免对 `no_sandbox` 会话误请求 sandbox stats；同一 conversation 的 `loadHistory()` / `loadWorkspaceTree()` 是 single-flight，请求未结束时不得再叠加新一轮拉取；点击“刷新当前对话”时，页面会暂停 history/workspace/sandbox 轮询，并以无默认超时的请求刷新当前 conversation 的 runtime，而不是新建会话
...
- **Agent Conversation feature** (`features/agent-conversation/`): 三列对话 UI (对话列表/消息流/上下文面板)。`NewConversationDraftPage` 复用 `ConversationComposer` 承载 `/agents/$agentId/conversations/new` 草稿态，首条消息调用 `POST /agent-definitions/:agentId/conversations/start` 并在成功后跳转真实 conversation 路由；`agentConversationStore` (Zustand) 管理消息列表与流式状态，页面进入时先通过 `GET /agent-conversations/:id/messages` 加载历史，再通过 Socket.IO `/agent-conversation` namespace 接续实时消息与 mid-stream injection。assistant 历史消息会保留 `toolCalls`，因此知识库检索结果可在消息列表中复现；若 assistant `metadata.subAgentStreams` 存在，child drill-in 会直接用这份 durable stream 恢复历史瀑布，而不是重新根据摘要拼装。`no_sandbox` Agent 的草稿态与正式会话都不会再额外渲染右侧提示面板，页面只保留消息流与输入区；对话收到顶层 `conversation.agent.done` 后会重新拉取一次消息历史，把数据库中的最终 assistant 正文作为 canonical 收口，避免纯工具调用轮次因没有 `message_chunk` 而丢失最终回复；执行中 `loadHistory()` / `loadWorkspaceTree()` 对同一 conversation 会共享单个 in-flight 请求，避免 3s 定时器把慢请求叠成连接风暴。`SandboxComputerPanel` 右上角会通过 `GET /agent-conversations/:id/sandbox/stats` 轮询 conversation 绑定沙箱的真实 CPU/MEM/DISK 值，`diskUsage=0` 时显示 `0 B`；执行开始时工具 tab 默认选中，但用户手动切到其他 tab 后不会再被后续工具事件抢回；若用户点击“刷新当前对话”，面板会暂停 `sandbox/stats` / `sandbox/processes` 轮询，直到当前 conversation 的 runtime refresh 完成或失败；历史对话继续执行时后端会自动比对 `execution.loadedPublishedVersionId` 与当前 published version，必要时丢弃旧 runtime session
...
- **organization-autonomy-policy feature** (`features/organization-autonomy-policy/`): `/settings/security/autonomy-policy` 复用 settings 风格布局与 token 解码门禁，仅允许 `owner`；组织 id **不来自 token claim**（实测 Supabase JWT 只有 `tenant_id/tenant_role`，且 tenantId 不是 organizationId），统一由 `useCurrentOrganization()`（`GET organizations/current`）解析，拿到真实 id 后才调用 `GET/PUT/POST /organizations/:id/autonomy-policy*`，支持“仅更新策略”、`downgrade-preview` 与 `downgrade-confirm` 两段式收紧流程；组织解析中只出骨架，解析失败（含 404 未归属组织）展示「无法确定当前组织」+ 重试且不发后续请求。当前组织策略卡片会展示 `organizationId`、`version`、`updatedAt` 与 `updatedBy` 元信息。共享 `autonomyModePolicy.ts` 提供 `AUTONOMY_MODES`、mode label/description、cap 比较和格式化 helper，供 settings 页、LLM panel 与优化建议阻断 UI 复用。
- **resource-governance feature** (`features/resource-governance/`): `/settings/resource-quotas` 复用 settings 风格布局与 token 解码门禁，允许 `owner/admin`；通过 `resourceGovernanceApi.ts` 对接 `GET /organizations/:id/resource-governance`、`PUT /organizations/:id/resource-governance/quota`、`PUT /organizations/:id/resource-governance/controls`、`POST /organizations/:id/resource-governance/executions/:executionId/terminate` 四个 API，其中 `:id` 来自 `useCurrentOrganization()`（`GET organizations/current`）。`ResourceGovernancePage` 提供 metadata、7 个 quota 字段表单、tenant/workflow governance controls、异常 execution termination 表单与 action summary；`resourceGovernancePermissions.ts` 只从 token 解析**角色**与**租户 id**（`getResourceGovernanceTenantIdFromToken`，无组织回退），并显式区分“治理暂停只阻止新执行进入”与 execution `paused`。
- **monitoring feature** (`features/monitoring/`): `/settings/monitoring` 复用 settings 风格布局与 `resource-governance` token 门禁，仅允许 `owner/admin`；通过 `monitoringApi.ts` 对接 `GET /organizations/:id/monitoring?window=15m|1h|24h`（`:id` 来自 `useCurrentOrganization()`），`monitoringKeys.dashboard(organizationId, window)` 显式把时间窗口放进 query key。页面由 `MonitoringDashboardPage` 组合 summary cards、`recharts` 执行趋势图、alerts/risk summary、metric source 说明与 hotspots；队列深度只在摘要/告警/热点中呈现当前 snapshot，不渲染成历史趋势曲线；页面保持只读，不暴露 quota/governance/termination mutation，并继续区分治理暂停与 execution `paused（人工介入）`。
...
- **DynamicConfigForm / AgentNodeConfigPanel / HttpToolConfigPanel**: 统一使用 react-hook-form + zodResolver + 300ms debounce `onApply`。workflow `agent` 节点的配置面板是 `AgentNodeConfigPanel`——选择已发布的 Agent Definition + 版本 + 输入映射；节点自身不承载内联 prompt / model 配置，系统提示词一律走 `text -> system-prompt-in` 连线。节点 data 上残留的 legacy 内联字段（`systemPrompt` / `system_prompt` / `model`，顶层优先、`data.config` 兜底）会以只读的「旧版内联 Agent 配置」区块展示（`data-testid="agent-legacy-inline-config"`），提示词可复制，仅作迁移参考，不写回也不删除。
...
- **浅色 / 深色 / 跟随系统三态主题**：`:root` 与 `:root.dark` 各持一套令牌值，默认跟随系统；节点类别色 `--color-node-*` 两套主题分别取值
- **Radix UI** accessible primitives + **CVA** (class-variance-authority)
- **cn()** = clsx + tailwind-merge
- **BEM-like** canvas 专用类在 `index.css`
- **prefers-reduced-motion** 覆盖
- **画布节点卡片视觉**: 卡片的层次感由令牌驱动——`--shadow-node` / `--shadow-node-selected` 为多层柔和投影，`.canvas-node-header` 用 `--node-color` 8% 顶部晕染向下淡出，`NodeIconChip` 用 25% 内描边，full header 的类型眉标取 60% 类别色混 `--color-muted`。节点 body 里凡是表达**通用语义轴**的颜色都必须取令牌而非 Tailwind palette 色阶：执行/健康等状态色走 `--color-success/warning/error/info`，端口数据类型色走 `--color-type-*`，节点类别色走 `--color-node-*`（palette 色阶在深浅主题下不会换值，状态点在深色主题里会偏暗）。各节点 body 内表达自身领域语义的装饰色（例如 `CodeToolNodeBody` / `ConditionNodeBody` / `HttpToolNodeBody` / `SandboxNodeBody` / `WebhookTriggerNodeBody` 里的语言标签、分支色、HTTP method 色）目前仍是 palette 类，未纳入本轮收口

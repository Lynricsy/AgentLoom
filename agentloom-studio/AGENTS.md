...
- **Trigger feature** (`features/trigger/`): `features/trigger/` 现要求普通 webhook config 只保证 `token + ipWhitelist`，`secret` 仅在创建成功返回中可选出现；`TriggerHistoryStatus` 已扩展 `signature_failed`；`WebhookConfigForm` 编辑态只展示 URL/Token 与“一次性 secret”提示，不再重复展示 secret；`API Event` 已收口为真正 preview-only：dialog 提交按钮禁用、表单只读、card 不允许编辑/启停，server 也会对 create/update/toggle 返回 409；当前 workflow 详情页仍然是 `WorkflowCanvasPage`（`/workflows/$workflowId`），不要为触发器发明独立 detail route

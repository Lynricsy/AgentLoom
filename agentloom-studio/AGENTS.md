...
| `/generated-apps/$appId`                         | GeneratedAppDetailPage         | Generated App 创建者详情页：AppSpec、验收场景、Gate 结果、Traceability、Artifacts、受控 workspace 源码/测试/报告预览、Gate 3 构建产物沙箱预览、Resource bindings、自动生成与验证运行、公开链接管理、公开提交记录管理与生成证据/运行记录查看；生成证据区默认选中最新可见 generation run |
...
│   ├── generated-app/ # 一句话生成应用 Studio 工作台与公开 runtime：creator API/hooks 管理 readiness/public share/submissions/generation runs/repair attempts/gate runs/start runner/受控 workspace artifact manifest + content；public API helpers 与内置公开页可提交/读取 submission，公开页只渲染 end-user runtime/submission 白名单字段、动态业务表单和结构化报告视图

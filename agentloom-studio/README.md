...
AgentLoom Studio 是基于 **React 19 + Vite 7** 的前端工作台，负责工作流画布编辑、执行调试、审计查询、治理配置、模板与市场浏览等 Web 体验。当前 `/login` 页面仅展示邮箱密码登录，Google / GitHub OAuth 的底层实现仍保留，但入口暂时隐藏；主侧边栏「探索」分组常驻「开发者」入口，指向 `/developer-console`（收益 / 开发者密钥两个 tab）。
...
| `/generated-apps/$appId`                         | GeneratedAppDetailPage         | Generated App 创建者详情页：AppSpec、验收场景、Gate 结果、Traceability、Artifacts、受控 workspace 源码/测试/报告预览、Gate 3 构建产物沙箱预览、Resource bindings、自动生成与验证运行、公开链接管理、公开提交记录管理与生成证据/运行记录查看；创建者提交详情优先展示 report handoff 状态，`pending/running/paused` 后台轮询时保留已加载详情并显示紧凑刷新提示；生成证据区会默认选中最新可见 generation run，让创建者直接看到对应修复尝试、结构化修复工作单、再验证计划与 Gate 证据                                                                                                                                                                                                                                                                                                      |
| `/generated-apps/public/$token`                  | GeneratedAppPublicRuntimePage  | Generated App 公开 runtime 入口：无需登录，只展示 title、description、dataUseNotice、有限 AppSpec、动态业务表单、runtime preview link 和结构化 submission 报告/结果，不渲染 Studio 壳层；当后端返回 `/api/v1/generated-apps/public/:token/preview` 时，API mapping 会在绝对 `VITE_API_BASE_URL` 部署下把预览链接解析到 API 域名，公开页只提供打开链接，不调用 creator artifact 接口、不渲染源码/测试 artifact 或 workspace metadata；打开后的 public build preview HTML 可走同源 public submission API 保存提交、展示报告并轮询 `pending/running/paused` handoff，API 不可用时保留本地 deterministic 报告                                                                                                                                                                                                                                     |

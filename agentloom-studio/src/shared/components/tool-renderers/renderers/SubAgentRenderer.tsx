...
const SubAgentSummary = memo(function SubAgentSummary({
  toolCall,
}: ToolSummaryProps) {
  const summary = useMemo(() => {
    switch (toolCall.tool) {
      case "call_subagent": {
        const alias = extractSubAgentAlias(toolCall);
        return `Call "${alias}"`;
      }
      case "spawn_subagent": {
        const alias = extractSubAgentAlias(toolCall);
...
const CallDetail = memo(function CallDetail({
  toolCall,
  state,
}: ToolRendererProps) {
  const alias = extractSubAgentAlias(toolCall);
...
const WaitDetail = memo(function WaitDetail({
  toolCall,
  state,
}: ToolRendererProps) {
  const args = safeParse<WaitArgs>(toolCall.args, {});
...
const StatusDetail = memo(function StatusDetail({
  toolCall,
  state,
}: ToolRendererProps) {
  const args = safeParse<StatusArgs>(toolCall.args, {});

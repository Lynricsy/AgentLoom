...
describe("MemoryInstancesPage", () => {
  ...
  it("默认显示顶部来源分类并按自己创建过滤", () => {
    ...
    expect(mocks.useMemoryInstances).toHaveBeenCalledWith(
      expect.objectContaining({ sourceKind: "manual" }),
    );
    expect(mocks.useAllMemoryInstances).toHaveBeenCalledWith({
      enabled: false,
      sourceKind: "manual",
    });
  });
  ...
  it("点击顶部来源分类后会切换为分享导入", async () => {
    ...
    expect(mocks.useMemoryInstances).toHaveBeenLastCalledWith(

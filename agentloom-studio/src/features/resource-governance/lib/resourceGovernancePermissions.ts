 ...
 * 资源治理的令牌权限判定。
 *
 * 这里只判定「令牌里的角色」与「令牌里的租户 id」。角色 claim 的来源、可靠性与
 * 「为什么前端读 claim 是安全的」，见
...
export type ResourceGovernanceAccessRole = Extract<InterventionRole, 'owner' | 'admin'>
...
function parsePayload(token?: string): ResourceGovernanceTokenPayload | null {
  if (!token) {
    return null
  }
  ...
  try {
    return JSON.parse(decoded) as ResourceGovernanceTokenPayload
  } catch {
    return null
  }
}
...
function collectRoleCandidates(payload: ResourceGovernanceTokenPayload | null): string[] {
  if (!payload) {
    return []
  }
...
export function getResourceGovernanceRoleFromToken(token?: string): InterventionRole | null {
...
export function canManageResourceGovernance(
  role: InterventionRole | null,
): role is ResourceGovernanceAccessRole {

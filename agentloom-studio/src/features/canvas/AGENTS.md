...
工作流可视化编辑器。基于 @xyflow/react (ReactFlow)，支持 14 种节点类型和端口兼容性检查。

## 节点类型体系

6 大类 14 种节点，注册在 `types/nodeTypeRegistry.ts`:
...
└── WorkflowCanvas.tsx (728L, 核心)
    ...
    │   └── NodeInfoOverlay.tsx
    ├── panels/
    │   ├── NodeConfigPanel.tsx (节点配置 + 实时输出 + 自定义面板/动态表单分发)
...
- `NodeConfigPanel` 配置分发规则：先命中自定义面板（llm-model/mcp-tool/knowledge-base/sandbox/llm-agent/http-tool），否则走 `DynamicConfigForm`，空 schema 显示“该节点无需额外配置”

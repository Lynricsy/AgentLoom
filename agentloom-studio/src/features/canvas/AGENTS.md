...
- 预览与编辑器共用节点组件，因此**预览态必须整体与编辑器全局状态隔离**：`components/PreviewModeContext.tsx` 由 `WorkflowPreviewCanvas` 提供（`edges` + `lodOverride`），`CanvasNodeShell` 在 context 非空时跳过 `useLlmApiKeys`（传 `enabled: false`）、执行态、校验态、搜索三态、hover store 写入、compound resize selector 与 `NodeExecutionOverlay`，并用 context 的 `edges` 代替 `canvasStore.edges`；`SmartRoutingNodeBody` 传 `useHealthStatus(false)`、`OutputNodeBody` 不读 executionStore、`ReusableBlockBody` 不渲染写 store 的展开按钮、`SmartEdge` 在 `data.readonlyPreview` 时不挂粒子；归一化结果的 node/edge 一律 `selected: false`。任何新增的节点 body 只要读受保护查询或全局 store，都必须走同一条 `usePreviewMode()` 隔离

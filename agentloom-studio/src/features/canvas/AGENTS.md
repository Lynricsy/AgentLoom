...
- `WorkflowPreviewCanvas` / `lib/workflowPreview.ts` 是非编辑场景工作流预览的单一事实源：会根据 `data.nodeType` 还原真实节点 category、对端口定义做与正式画布一致的 hydration，并把预览 edge 统一映射为只读 `smart` edge。坏数据兜底也必须落在真实卡片上——未知 `nodeType` 归一化为 `type: 'control'` + `data.nodeType='unknown-node'`（由 `getResolvedNodeTypeConfig()` 渲染成「未知节点类型」muted 卡片），缺 `position` 的节点排进 3 列 320×200 兜底网格而不是被丢弃；只有缺 `id` 才丢弃。禁止再产出 `type: 'default'`，那会命中 `index.css` 里被清空样式的 `.react-flow__node-default`
- 预览与编辑器共用节点组件，因此**预览态不得让编辑器状态影响渲染，也不得写回编辑器 store**。机制分两种，别混：受保护查询靠 `enabled` 真正不发请求（`useLlmApiKeys({ enabled: false })`、`useHealthStatus(false)`）；其余 store 订阅**仍无条件调用以保持 hook 顺序**，只是结果被丢弃或不渲染——`CanvasNodeShell` 照旧调 `useNodeExecutionState` / `useNodeHasValidationError` / 搜索三态 selector，但在 previewMode 下分别覆盖成 `undefined` / `false` / `false`，compound resize selector 首行返回 `null`，hover 回调首行 return，`NodeExecutionOverlay` 整体不挂载，`canvasEdges` 取 context 的 `edges` 而非 `canvasStore.edges`；`OutputNodeBody` 同样仍订阅 `useNodeExecutionState`，只是把值置空后走空态；`ReusableBlockBody` 不渲染写 store 的展开按钮；`SmartEdge` 仍订阅目标节点执行态，但 `data.readonlyPreview` 时不挂粒子；归一化结果的 node/edge 一律 `selected: false`。任何新增的节点 body 只要读受保护查询或全局 store，都必须走同一条 `usePreviewMode()` 隔离
- `useLevelOfDetail()` 支持 `PreviewModeContext.lodOverride` 强制层级：小容器 `fitView` 会把 zoom 压到 0.4 以下，缩略图仍需完整卡片。template / marketplace 详情预览传 `lodOverride="full"`，全屏公开分享页不传（保留 zoom 驱动 LOD）；`WorkflowPreviewCanvas` 同时把 `minZoom` 放宽到 0.05，让 fitView 能真正装下整图
...
- `CanvasNode` 现在有 3 档 LOD：`full (>=0.7)` / `compact (0.4–0.7)` / `minimal (<0.4)`；minimal 模式应保持图标方块 + 可连线 handles，不渲染 body、port row 与 execution overlay。节点 body 只在 `full` 下渲染，因此 body 内部**不得**再按 `useViewport().zoom` 做二次 high/medium/low 降级——那些分支在编辑器不可达，只会在预览的 `lodOverride='full'` 下意外降级内容
...
- 节点外壳的边框 / 阴影 / 选中 ring / running conic 描边、头部类别色晕染（`.canvas-node-header`）、端口行 hover（`.port-row:hover`）与端口抖动（`.typed-port--shake`）全在 `index.css` 的节点外壳段，组件只通过内联 `--node-color` 传色（`NodeIconChip` 的内描边与 full header 眉标色也由该变量派生）；`NodeExecutionOverlay` 常驻卡片右上角，头部徽章区靠 `pr-8` 让位，改任一侧都要同步另一侧
- `.typed-port--shake` 的关键帧动画 `translate` 属性而非 `transform`：端口的 transform 链全是 `translateY(-50%) … !important`（`.port-row .typed-port` / `.typed-port:hover` / 各 `typed-port-state--*`），`!important` 声明会压掉动画，独立的 `translate` 属性在 transform 之前应用并与之叠加，不参与那条竞争
...
- `SmartEdge` 粒子只在**目标节点 `status === 'running'`** 时挂载（与 `useExecutionHighlight` 的 dep-active 语义一致），空闲画布上的边一律静止；`error` / `checking` 任何时候都不出粒子；`data.readonlyPreview` 的预览边也一律静止，避免同 id 的编辑器执行态漏进预览

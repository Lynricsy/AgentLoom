...
const BUILTIN_PRESETS: SandboxPreset[] = [
  { id: 'builtin-light', name: '轻量', cpu: 0.5, memory: 512, disk: 2, isBuiltin: true },
  { id: 'builtin-standard', name: '标准', cpu: 1, memory: 1024, disk: 5, isBuiltin: true },
  { id: 'builtin-performance', name: '高性能', cpu: 2, memory: 2048, disk: 10, isBuiltin: true },
]
...
export const useSandboxPresetStore = create<SandboxPresetState & SandboxPresetActions>()(
  devtools(
    persist(
      immer((set) => ({
        customPresets: [],
...
export function findMatchingPreset(
  ...
  return allPresets.find(
    (p) => p.cpu === config.cpu && p.memory === config.memory && p.disk === config.disk,
  )
}

...
type StatsQueryWithRefetchInterval = StatsQuery & {
  options: StatsQuery["options"] & {
    refetchInterval?: ((query: StatsQuery) => false | number) | false | number;
  };
};
...
describe("useSandboxes", () => {
  ...
  it("存在 creating/stopping 沙箱时应持续轮询列表", async () => {
    ...
    if (
      !typedQuery ||
      typeof typedQuery.options.refetchInterval !== "function"
    ) {
...
describe("useSandboxStats", () => {
  ...
  it("运行中的沙箱会拉取资源统计", async () => {
    ...
    const { result } = renderHook(() => useSandboxStats("session-1", "ready"), {
      wrapper: Wrapper,
    });

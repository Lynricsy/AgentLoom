...
function getShareTypeBadgeClass(shareType: PublicShareData['shareType']): string {
...
function getResourceTypeLabel(resourceType: PublicShareData['resourceType']): string {
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-emerald-500/20 bg-background/80 p-4">
        ...
        <div className="grid gap-2 sm:grid-cols-4">
          <div className="rounded-lg border border-border/60 bg-card p-3">
            ...
            <div className="mt-1 text-lg font-semibold">{result.summary.cloned}</div>
          </div>
          <div className="rounded-lg border border-border/60 bg-card p-3">
            <div className="text-xs text-muted-foreground">已清空</div>
            <div className="mt-1 text-lg font-semibold">{result.summary.cleared}</div>
          </div>
          <div className="rounded-lg border border-border/60 bg-card p-3">
            <div className="text-xs text-muted-foreground">待重绑</div>
            <div className="mt-1 text-lg font-semibold">{result.summary.needsRebind}</div>
          </div>
          <div className="rounded-lg border border-border/60 bg-card p-3">
            <div className="text-xs text-muted-foreground">跳过临时资源</div>
            <div className="mt-1 text-lg font-semibold">{result.summary.skippedEphemeral}</div>
          </div>
        </div>
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <span
                  className={cn(
                    ...
                    item.outcome === 'cloned' && 'border-emerald-500/20 bg-emerald-500/10 text-emerald-400',
                    item.outcome === 'cleared' && 'border-amber-500/20 bg-amber-500/10 text-amber-400',
                    item.outcome === 'needs_rebind' && 'border-rose-500/20 bg-rose-500/10 text-rose-400',
                    item.outcome === 'skipped_ephemeral' && 'border-slate-500/20 bg-slate-500/10 text-slate-400',
                  )}
                >
                  {item.outcome === 'cloned'
                    ? '已复制'
...
export function PublicSharePage() {
  ...
  const previewNodes = useMemo(() => (data?.definition.nodes ?? []) as Node[], [data])
  const previewEdges = useMemo(() => (data?.definition.edges ?? []) as Edge[], [data])
  ...
  const description =
    data.resourceType === 'agent' ? data.agentDescription : data.workflowDescription
  ...
  const handleImport = () => {
    ...
    if (data.resourceType === 'workflow') {
      createWorkflowMutation.mutate(
        ...
        {
          onSuccess: (result) => {
            notify({ description: '已导入到你的工作流列表', variant: 'success' })
            void navigate({
    ...
    importAgentMutation.mutate(token, {
      ...
      onError: () => {
        notify({ description: 'Agent 导入失败，请确认已登录', variant: 'error' })
      },
    })
  }
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background" data-testid="public-share-page">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
              ...
              {data.expiresAt ? <span>有效期至 {formatDateTime(data.expiresAt)}</span> : null}
              <span>{data.nodeCount} 个节点</span>
              <span>{data.edgeCount} 条连线</span>
              {agentMeta ? <span>运行模式: {agentMeta.runtimeMode}</span> : null}
              {agentMeta?.sandboxLifecycle ? <span>沙箱生命周期: {agentMeta.sandboxLifecycle}</span> : null}
            </div>
          </div>
          ...
          {data.shareType === 'copyable' ? (
            ...
            >
              {importPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileUp className="h-4 w-4" />}
              {data.resourceType === 'agent' ? '导入到我的 Agent' : '复制到我的工作流'}
            </Button>
          ) : null}
        </div>
      </header>
      ...
      <div className="flex-1">
        <ReactFlow
          nodes={previewNodes}
          edges={previewEdges}
          defaultViewport={data.definition.viewport ?? DEFAULT_VIEWPORT}
          fitView

...
export async function fetchProviders(): Promise<LlmProviderEntity[]> {
  const res = await apiClient.get('llm-providers').json<ApiResponse<LlmProviderEntity[]>>()
  return res.data
}

/** 获取单个 Provider */
export async function fetchProvider(id: string): Promise<LlmProviderEntity> {
  const res = await apiClient.get(`llm-providers/${id}`).json<ApiResponse<LlmProviderEntity>>()
  return res.data
}

/** 创建自定义 Provider */
export async function createProvider(input: CreateLlmProviderInput): Promise<LlmProviderEntity> {
...
export async function resetProviderBaseUrl(id: string): Promise<LlmProviderEntity> {
  const res = await apiClient
...
/** 从 Provider 发现可用模型 */
export async function discoverProviderModels(id: string): Promise<DiscoveredModel[]> {
  const res = await apiClient
...
export async function searchProviderLiteLLMModels(id: string): Promise<LiteLLMModelInfo[]> {
  const res = await apiClient
...
export async function fetchLlmModels(): Promise<LlmModelConfigEntity[]> {
  const res = await apiClient.get('llm-models').json<ApiResponse<LlmModelConfigEntity[]>>()
  return res.data
}

/** 获取单个模型配置 */
export async function fetchLlmModel(id: string): Promise<LlmModelConfigEntity> {
  const res = await apiClient.get(`llm-models/${id}`).json<ApiResponse<LlmModelConfigEntity>>()
  return res.data
}

/** 创建模型配置 */
export async function createLlmModel(input: CreateLlmModelInput): Promise<LlmModelConfigEntity> {
  const res = await apiClient

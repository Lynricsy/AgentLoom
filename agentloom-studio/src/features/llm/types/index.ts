...
/**
 * LlmModelInfo -- 兼容旧版消费方的模型信息。
 *
 * 旧代码依赖 `provider`(slug string) / `modelName`(string) 字段，
 * 但新版服务端响应里的 `provider` 是嵌套对象。
 *
 * 为避免同一个字段同时承担“对象”和“字符串”两种语��，这里把
 * 服务端原始 provider 对象保存在 `providerEntity`，并把 `provider`
 * 规范化为旧版仍可直接消费的 slug 字符串。
 */
export type LlmModelInfo = Omit<LlmModelConfigEntity, "provider"> & {
  /** 兼容旧版：provider slug 字符串 */
  provider: string;
  /** 兼容旧版：模型标识符 (等同于 modelId) */
  modelName: string;
  /** 新版服务端 provider 实体，供需要 URL / API Key 等元信息的组件使用 */
...
export function normalizeLlmParameters(value: unknown): LlmParameters {
  ...
  return {
    temperature: parseNumber(
      record.temperature,
      DEFAULT_LLM_PARAMETERS.temperature,
    ),
    maxTokens:
...
export function parseLlmModelConfig(value: unknown): LlmModelConfig | null {
  ...
  const modelName =
    parseString(source.modelName) ?? parseString(source.modelId);
  ...
  const providerObj =
    typeof source.provider === "object" && source.provider !== null
      ? (source.provider as Record<string, unknown>)
      : null;
  const endpointUrl =
    parseNullableString(source.endpointUrl) ??
    (providerObj ? parseNullableString(providerObj.baseUrl) : null);

  return {
    llmConfigId:
      parseNullableString(source.llmConfigId) ?? parseNullableString(source.id),
    ...
    apiKeyId:
      parseNullableString(source.apiKeyId) ??
      (providerObj ? parseNullableString(providerObj.apiKeyId) : null),
    embeddingDimensions:
      typeof source.embeddingDimensions === "number" &&
      Number.isFinite(source.embeddingDimensions)
    ...
    authConfig:
      typeof source.authConfig === "object" && source.authConfig !== null
...
export function toLlmModelConfig(model: LlmModelInfo): LlmModelConfig {
  const providerObj = model.providerEntity ?? null;
  const providerSlug = providerObj?.slug ?? model.provider;
 ...
 */
export function adaptModelEntityToInfo(
  entity: LlmModelConfigEntity,
): LlmModelInfo {
  return {
    ...entity,

...
describe("ProviderIcon", () => {
  ...
  it("deprecated provider prop still works and resolves product color assets", () => {
  ...
  it("slug takes precedence over provider", () => {
    ...
    expect(img).toHaveAttribute(
      "src",
      "https://unpkg.com/@lobehub/icons-static-png@latest/dark/deepseek-color.png",
    );
  });

  it("iconUrl overrides the CDN URL", () => {
    render(
      <ProviderIcon slug="custom" iconUrl="https://example.com/icon.svg" />,
    );
    const img = screen.getByRole("img");
  ...
  it('resolves to "unknown" when neither slug nor provider given', () => {
    ...
    expect(img).toHaveAttribute(
      "src",
      "https://unpkg.com/@lobehub/icons-static-png@latest/dark/unknown.png",
    );
  ...
  it("null iconUrl falls back to CDN URL", () => {
    ...
    expect(img).toHaveAttribute(
      "src",
      "https://unpkg.com/@lobehub/icons-static-png@latest/dark/gemini-color.png",
    );
  });

  it("rewrites legacy icons.lobehub.com URLs to the product color CDN asset", () => {
    render(
      <ProviderIcon
        slug="anthropic"
  ...
  it("maps siliconflow to the siliconcloud color asset", () => {
    render(<ProviderIcon slug="siliconflow" />);
    const img = screen.getByRole("img");
    expect(img).toHaveAttribute(
      "src",
      "https://unpkg.com/@lobehub/icons-static-png@latest/dark/siliconcloud-color.png",
    );
  });

  it("normalizes managed png iconUrl back to the dark color asset", () => {
    render(
      <ProviderIcon
        slug="anthropic"
        iconUrl="https://unpkg.com/@lobehub/icons-static-png@latest/light/claude-color.png"
      />,
    );
    const img = screen.getByRole("img");
    expect(img).toHaveAttribute(
  ...
  it("skips remote icon fetch for private_cloud when no custom iconUrl is provided", () => {
    render(<ProviderIcon slug="private_cloud" />);
    expect(screen.getByTestId("icon-bot")).toBeInTheDocument();
    expect(screen.queryByRole("img")).not.toBeInTheDocument();
  });

...
describe('versionMutations', () => {
  it('createVersion 会提交 snake body 并等待列表失效完成', async () => {
    ...
    const { result } = renderHook(() => useCreateVersion('wf-001'), { wrapper })

    let settled = false
    let mutationPromise!: Promise<WorkflowVersion>

    await act(async () => {
      mutationPromise = result.current.mutateAsync({ label: 'v1' }).then((value) => {
        settled = true
        return value
      })
    })

    expect(postMock).toHaveBeenCalledWith('workflow-definitions/wf-001/versions', {
      json: { label: 'v1' },
    })
  ...
  it('rollback 会等待 workflow detail 与 version all 的失效', async () => {
    ...
    const { result } = renderHook(() => useRollbackVersion('wf-001'), { wrapper })
  ...
  it('publish 会提交 releaseNotes 并等待所有失效完成', async () => {
    ...
    await act(async () => {
      mutationPromise = result.current
        .mutateAsync({ label: '发布版', releaseNotes: '补充发布说明', versionId: 'ver-003' })
        .then((value) => {
          settled = true
          return value
        })
    ...
    expect(postMock).toHaveBeenCalledWith('workflow-definitions/wf-001/publish', {
      json: { label: '发布版', releaseNotes: '补充发布说明', versionId: 'ver-003' },
    })
  ...
  it('archive 会等待所有失效完成', async () => {
    ...
    const { result } = renderHook(() => useArchiveWorkflow('wf-001'), { wrapper })
    ...
    expect(postMock).toHaveBeenCalledWith('workflow-definitions/wf-001/archive')

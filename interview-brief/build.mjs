#!/usr/bin/env node
/**
 * 把 src/*.html 片段拼成单文件 HTML，再用 headless Chrome 打成 A4 PDF。
 * 体检：纵向溢出 + 任意后代越出页面内容盒（横向裁切）。
 */
...
(function(){
  ...
  pages.forEach(function(pg, i){
    ...
    if(!pg.hasAttribute('data-nochrome')){
      ...
      c.className = 'chrome';
      c.innerHTML = '<div class="l">AgentLoom · 系统设计复盘手册</div>' +
  ...
  // 目录页码：data-ref="A1" 匹配 data-sec 为 "A1" 或以 "A1 " 开头的页
  Array.prototype.forEach.call(document.querySelectorAll('[data-ref]'), function(el){
    ...
    if(hit < 0) (window.__tocMiss = window.__tocMiss || []).push(ref);
  });
  ...
  window.__report = { overflow: bad, pages: total, tocMiss: window.__tocMiss || [] };
})();
</script>
</body></html>`;

const parts = readdirSync(srcDir).filter((f) => f.endsWith('.html')).sort();
const html = parts.map((f) => readFileSync(join(srcDir, f), 'utf8')).join('\n') + runtime;
...
const pdfPath = join(outDir, 'AgentLoom-系统设计复盘手册.pdf');
await page.pdf({ path: pdfPath, printBackground: true, preferCSSPageSize: true });
await browser.close();

console.log(`pages: ${report.pages}  ->  ${pdfPath}`);
if (report.tocMiss && report.tocMiss.length) {
  console.log('TOC UNRESOLVED: ' + report.tocMiss.join(', '));
  process.exitCode = 1;
}
if (report.overflow.length) {
  console.log(`ISSUES (${report.overflow.length} pages):`);

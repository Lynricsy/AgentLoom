...
- **🔗 发现与公开分享** — `/discover` 复用 Marketplace 已上架内容做可浏览发现页；workflow 与 Agent 都支持生成 `/s/:token` 公开分享链接，访问者可预览作者、标题、简介、画布/Agent 元数据，并导入到自己的租户。Generated App 使用 `/generated-apps/public/:token` 作为无需登录的公开 runtime 入口，只暴露终端用户业务界面、数据用途提示、有限 AppSpec、由 AppSpec/静态合约派生的动态业务表单、结构化报告视图与预览链接；公开 runtime 会渲染所有白名单 `runtimeForm.fields[]`，未被 section 引用的生成字段会进入兜底分组，避免必填字段不可见导致无法提交；Gate 3 build output 可读时，预览链接会指向 `GET /api/v1/generated-apps/public/:token/preview`，该公开端点只返回 `dist/index.html` HTML，HTML 在公开预览路径下只可调用同源 public submission create/detail API 来保存提交、展示报告并轮询 `pending/running/paused` handoff，不在公开路径或 API 不可用时退回本地 deterministic 预览，且不暴露源码、测试报告、artifact 清单、workspace metadata 或 host 绝对路径；生成过程会保存自动开发测试循环、修复轮次、Gate 0-7 运行证据、失败详情和修复建议，Gate 3 `real-local-command-plan` 首次失败且修复预算大于 0 时会创建 running 修复尝试、应用服务端受控 frontend workspace repair patch、记录 linked attempt-2 Gate 3 再验证运行并按结果完成或失败该修复尝试；其他暂无可执行补丁路径的 failed gate 仍会写入一条 failed no-patch 修复尝试台账，并附带结构化 `repairPlan` / `reverificationPlan` 作为后续补丁与再验证的机器可读工作单；retry 运行会把最近 failed repair attempt 写入 `generationPlan.repairContext` 和 Gate 1 证据作为本轮修复目标，后端可同步启动轻量门禁运行器执行 Gate 0 AppSpec、Gate 1 架构计划、Gate 2 静态合约、Gate 3 build/unit workspace runner、Gate 4 integration runner、Gate 5 browser acceptance runner、Gate 6 independent verifier runner 和 Gate 7 publish-candidate contract runner。
  Gate 3 生成的 React/Vite 预览和 `dist/index.html` 静态预览同样把 `runtimeForm.fields[]` 作为完整输入合约，未被 `sections[].fieldIds` 引用的字段会进入 `其他信息` 兜底分组后再参与校验和提交。
  当 AppSpec 暗示问诊、评分、校验、风险筛查、逐步追问、工具或 API 型处理时，Gate 1 会规划租户私有 generated plugin tool，并要求 manifest validation、build、signature verification、permission policy、sandbox smoke 与 safety scan 硬门禁；Gate 3 会在受控 workspace 下生成 `plugins/<toolId>/` 的插件 manifest、节点定义、源码、smoke fixture、真实 `dist/plugin.wasm`、声明 `wasmEntry` 的签名 `.alp` 和 build report；Gate 7 real-local 通过后，服务端会重新校验 `.alp` 的 manifest/content hash/RSA-PSS 签名、零权限声明、WASM 入口和硬门禁，上传 `.alp` 与 WASM bundle，再注册新插件或刷新同租户既有插件记录的 archive/signature/content hash/metadata/`wasmBundleUrl`，激活为 `tenant-private`，把插件数据库 id 写入 Generated App。Gate 7 生成的 runtime Workflow 会把这些私有工具作为 `plugin` 节点接入 execution DAG，公开提交触发异步 Workflow 后由 `PluginExecutionWorker` 在租户事务内更新 step 状态并回调调度器；带 `wasmBundleUrl` 的生成私有插件走 `PluginSandboxService` / Extism 执行链，只有遗留无 WASM 的 generated-private artifact 在 metadata `source`、`activationScope` 和 `toolId` guard 全部匹配时才允许受控 deterministic fallback 输出结构化 `analysis`。插件 bundle 会出现在创建者侧 allowlist artifact manifest 中，但不会通过公开 runtime、公开 submission、公开预览或 marketplace 暴露。
  Gate 3 `real-local-command-plan` 只通过 `shell=false` 执行服务端 allowlist Node 脚本并脱敏输出；Gate 4 `real-local-integration` 只执行受控 deterministic public runtime、creator query、Agent/Workflow local trace fixture 与插件 local smoke contract；Gate 5 按 `GENERATED_APP_GATE5_EXECUTOR_MODE` / `APP_GENERATED_APP_GATE5_EXECUTOR_MODE` 选择 `real`、`real-browser-e2e`、`fixture` 或 `disabled`。`real-local-browser-contract` 只执行受控 deterministic 本地 DOM/accessibility/network/console contract，并要求覆盖公开 runtime open/submit/detail 与 public build preview submit 旅程，不启动 Playwright 或真实浏览器，不访问真实公开链接，也不捕获真实截图、视频或 trace；`real-browser-e2e` 是服务端受控真实浏览器 E2E runner contract，只允许访问 `/generated-apps/public/:token` runtime/preview surface，覆盖 open -> fill -> submit -> detail/report，禁止 creator APIs、内部 artifacts、host path、plugin ids、workflow snapshots、step/checkpoint/raw tool data 和 secret 泄漏；Playwright 或浏览器不可用时会 fail-closed 写入 unavailable evidence，`GENERATED_APP_GATE5_REAL_BROWSER_UNAVAILABLE_REASON` / `APP_GENERATED_APP_GATE5_REAL_BROWSER_UNAVAILABLE_REASON` 可显式声明不可用原因，不会用 fixture 或 local contract evidence 伪装真实 E2E 通过。Gate 6 按 `GENERATED_APP_GATE6_EXECUTOR_MODE` / `APP_GENERATED_APP_GATE6_EXECUTOR_MODE` 选择 `real`、`fixture` 或 `disabled`；`real-local-independent-verifier` 是受控 deterministic 本地规则 verifier，只读取 redacted evidence bundle、Gate 0-5 evidence refs、rubric 与 coverage matrices，输出 `blockingFindings`、`warnings`、`decision`、`traceabilityCoverage`、`repairSuggestions`、`residualRiskSummary`，不访问外部网络，不调用任意模型，不读取 generation transcript/public share token/API key/secret，也不代表外部模型或人工审查；fixture 只标记 verdict shape 且 `executed=false`，disabled 会失败并停止后续门禁。
  Gate 7 按 `GENERATED_APP_GATE7_EXECUTOR_MODE` / `APP_GENERATED_APP_GATE7_EXECUTOR_MODE` 选择 `real`、`fixture` 或 `disabled`；`real-local-publish-candidate-contract` 是受控 deterministic 本地发布候选合约 runner，只签收 release manifest contract、checksum placeholders、Gate 0-6 evidence citations 与 deferred public-share controls，不执行任意 shell/用户路径，不创建生产发布、真实 artifact archive、真实签名、外部 verifier 结果或 public share token。Gate 3-6 任一为 fixture/disabled/skeleton 时 Gate 7 会阻断；Gate 3、Gate 4、Gate 6 均为 real-local，Gate 5 为 `real-local-browser-contract` 或具备 `executed=true` / `playwrightExecuted=true` / `realBrowserExecuted=true` 匹配 runner evidence 的 `real-browser-e2e`，且 Gate 7 real runner 通过后，generation run 可为 `passed`，readiness 成为 `publish_candidate`，并创建或复用同租户已发布 Generated App runtime Workflow 作为公开提交的异步 execution 目标，但公开 token 仍为 null，只有后续显式启用公开分享才会通过 readiness guard 创建 token。Gate 4/5/6 real-local evidence 只证明对应受控 deterministic 本地 contract/rules runner，不证明生产 sandbox run、Playwright run、外部独立模型审查、真实独立代理审查、真实人工审查或完整端到端需求满足判定；Gate 5 real-browser-e2e unavailable evidence 也只证明真实 E2E runner contract 被请求且 fail-closed，不证明真实浏览器旅程已通过；Gate 7 real-local evidence 不代表生产级 artifact archive/signature/upload 已完成。公开提交通过动态业务表单收集终端用户输入，会先对 token-like/secret/host path/unsupported 结构做脱敏或失败收口，再同步生成 `local-generated-app-deterministic-report` 类型的本地 deterministic result/report 并持久化到创建者租户；若绑定 Workflow 为同租户、已发布且存在 `publishedVersionId`，还会创建异步 Workflow execution，并且公开响应只追加 execution id/status/boundary 等安全 handoff 字段；公开/创建者提交详情会在租户范围内刷新 handoff，优先使用 `report` 中的 handoff 选择 execution，持久化脱敏后的 execution 状态和安全摘要；当 execution 完成时，公开摘要可展示经过白名单过滤的业务输出（如私有插件 `analysis`、`text-output.content` 或 `json-output.json`），但仍不暴露 step 原文、node/checkpoint、tool calls、definition snapshot、插件 ID、artifact URL、路径、token 或 secret；`pending`、`running`、`paused` 都作为未终止状态继续轮询，创建者提交列表会刷新当前页未终止 handoff；该报告在公开页以结构化摘要、下一步问题、补充提示和边界说明呈现，不默认展示内部 JSON dump，也不伪装为真实 AI、生产 sandbox 或插件执行；医疗/问诊类内容只生成摘要、下一步问题和非诊断免责声明。创建者可按应用查看提交列表/详情并单条或批量删除，也可在创建者工作台通过受控 workspace 产物清单查看 Gate 3 生成的源码、runtime form、测试、报告内容和 `dist/index.html` 自包含交互式业务表单预览；这些交付物只按服务端 allowlist artifactId 读取，不向公开 runtime 暴露 host 绝对路径。Studio 中 Agent 画布提供独立的保存版本、历史记录、发布工具栏，且未发布 Agent 不暴露分享入口
...
| `/generated-apps/public/$token` | Generated App 公开 runtime 动态业务表单、运行预览链接与结构化报告入口      |

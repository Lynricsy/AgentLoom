...
- **🔗 发现与公开分享** — `/discover` 复用 Marketplace 已上架内容做可浏览发现页；workflow 与 Agent 都支持生成 `/s/:token` 公开分享链接，访问者可预览作者、标题、简介、画布/Agent 元数据，并导入到自己的租户。Generated App 使用 `/generated-apps/public/:token` 作为无需登录的公开 runtime 入口，只暴露终端用户业务界面、数据用途提示、有限 AppSpec 与预览链接；生成过程会保存自动开发测试循环、修复轮次、Gate 0-7 运行证据、失败详情和修复建议，后端可同步启动轻量门禁运行器执行 Gate 0 AppSpec 完整性检查，Gate 0 通过后生成结构化 `generationPlan` 并执行 Gate 1 架构计划完整性检查，Gate 1 通过后生成 `generationPlan.staticContracts` 并执行 Gate 2 静态合约完整性检查，Gate 2 通过后生成包含受控 `generationWorkspace` contract、`commandPlan` 和执行层级的 `generationPlan.buildUnitPlan`；Gate 3 默认在服务端受控 workspace materialize deterministic React/Vite/TypeScript 工程骨架，`real-local-command-plan` 只通过 `shell=false` 执行服务端 allowlist Node 脚本，拒绝未知 command id、任意 shell、绝对脚本路径、traversal cwd 或命令字符串不匹配，并保存 command、exitCode、已脱敏 stdout/stderr 摘要、artifact refs 和 requirement/scenario evidence，`fixture-execution` 会明确标记 `executed=false`，`disabled-execution` 会失败并停止后续门禁。Gate 3 通过后生成 `generationPlan.integrationPlan` 并执行 Gate 4 集成计划完整性检查，随后按 `GENERATED_APP_GATE4_EXECUTOR_MODE` / `APP_GENERATED_APP_GATE4_EXECUTOR_MODE` 选择 `real`、`fixture` 或 `disabled` runner；`real-local-integration` 会执行受控 deterministic public runtime read/submit/detail payload contract、creator query whitelist contract、Agent/Workflow local trace fixture 和插件 local smoke trace fixture，不执行任意 shell/用户路径，也不代表生产 sandbox 或真实 Extism/WASM；`fixture-integration` 只标记 trace shape 且 `executed=false`，`disabled-integration` 会失败并停止后续门禁。Gate 4 通过后生成 `generationPlan.browserAcceptancePlan` 并按 `GENERATED_APP_GATE5_EXECUTOR_MODE` / `APP_GENERATED_APP_GATE5_EXECUTOR_MODE` 选择 `real`、`fixture` 或 `disabled` runner；`real-local-browser-contract` 执行受控 deterministic 本地 DOM/accessibility/network/console contract，不执行任意 shell/用户路径，不启动 Playwright 或真实浏览器，不访问真实公开链接，也不捕获真实截图、视频或 trace；`fixture-browser-acceptance` 只标记 assertion shape 且 `executed=false`，`disabled-browser-acceptance` 会失败并停止后续门禁。Gate 5 通过后生成 `generationPlan.independentVerificationPlan` 并执行 Gate 6 独立审查 skeleton 完整性检查，Gate 6 通过后生成 `generationPlan.publishCandidatePlan` 并执行 Gate 7 publish-candidate guard skeleton；当前 Gate 7 在检测到 Gate 6 仍为 skeleton/contract-level evidence、缺少真实 independent verifier artifact 签收与真实独立 verifier verdict，或 Gate 5 处于 fixture/disabled/skeleton evidence、Gate 4 处于 fixture/disabled/skeleton evidence、Gate 3 处于 fixture/disabled/contract-skeleton evidence 时写入 failed Gate 7 evidence，generation run 保持 `failed`，readiness 不会成为 `publish_candidate`，公开 token 会关闭并清空；Gate 4 real-local evidence 只证明受控 deterministic 本地 contract runner，不证明生产 sandbox run、真实 Agent/Workflow sandbox 执行或真实插件 WASM/Extism smoke test；Gate 5 real-local evidence 只证明受控 deterministic 本地 browser contract runner，证据记录 assertion/journey/viewport、console/network summary、artifact refs 与覆盖引用，不证明 Playwright run、真实浏览器会话、真实截图/视频/trace 捕获、真实公开链接访问或完整端到端浏览器执行；Gate 6 只表示 verifier 隔离策略、redacted evidence bundle、Gate 0-5 evidence ids、rubric、verdict schema、independence checks、需求/场景/evidence/gate 覆盖和失败捕获字段的 independent-verifier-skeleton 完整，不代表真实独立模型审查、真实独立代理审查、真实人工审查、真实运行结果判定或真实需求满足判定已经执行；Gate 7 只表示 publish-candidate guard skeleton 检查结果，不代表真实发布候选已生成、真实 artifact 已签收、真实质量门禁全量通过或可以公开分享，创建者可在 Studio 详情页查看 generation run、repair attempt 与 Gate run 证据摘要；公开提交内容、运行结果占位和最终报告占位会持久化到创建者租户，创建者可按应用查看提交列表/详情并单条或批量删除。Studio 中 Agent 画布提供独立的保存版本、历史记录、发布工具栏，且未发布 Agent 不暴露分享入口

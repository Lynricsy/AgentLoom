...
- Public share UI:
  ...
  - The public runtime page must render every whitelisted `runtimeForm.fields[]` entry at least once. Fields referenced by `sections[].fieldIds` render in their sections; fields not referenced by any section render in a local fallback section so required validation never blocks on an invisible generated field.
...
- Creator artifact delivery:
  ...
  - When the manifest contains readable artifact `gate-3-build-output-html`, the Artifacts panel must also render it as the creator-side Gate 3 build preview in a sandboxed iframe using `srcDoc` and `sandbox="allow-scripts"`. The sandbox allows the generated deterministic form/report script to run but must not grant `allow-same-origin`, forms, popups, downloads, top-navigation, or parent access. Missing or unreadable build output shows a local empty/unavailable state instead of guessing paths or falling back to legacy preview URLs.
...
- Public submissions API boundary:
  ...
  - The built-in `GeneratedAppPublicRuntimePage` renders the whitelisted `runtimeForm` as the end-user business input surface, including required validation, unsectioned-field fallback rendering, and `text | textarea | single_select | multi_select | number | range` field controls.
...
- Good: a public runtime page shows data-use notice, public AppSpec summary, a dynamic business form from `runtimeForm`, optional runtime preview link, and structured public submission report/status without rendering Studio navigation, internal JSON dumps, or the token value; generated fields omitted from section references remain visible in a fallback section and can be submitted.
...
- Component tests:
  ...
  - public runtime page renders data-use notice, limited AppSpec, optional preview link, dynamic `runtimeForm` controls including fields missing from section references, required validation, submitted payload, structured result/report sections, and does not render tokens or internal fields.

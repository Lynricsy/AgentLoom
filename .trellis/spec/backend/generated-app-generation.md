...
- Readiness rules:
  ...
  - A `publish_candidate` readiness computed from `gateResults` must also pass the service-side publish candidate evidence guard before it is persisted. The guard must reject manual, fixture, disabled, skeleton, malformed, unavailable/not-implemented, or stale evidence that lacks trusted Gate 5 runner evidence for the current browser execution level or trusted Gate 7 real-local publish-candidate runner details, even when all canonical blocking gates are marked `passed`. For Gate 5 `real-browser-e2e`, trusted evidence requires the matching `gate-5-real-browser-e2e-runner` details with `executed=true`, `playwrightExecuted=true`, and `realBrowserExecuted=true`.
- Gate run recording:
  ...
  - If recomputation would produce `publish_candidate`, the service must first validate the current or submitted `generationPlan.publishCandidatePlan` plus Gate 5 and Gate 7 evidence: Gate 3, Gate 4, and Gate 6 execution levels must be real-local, Gate 5 must be `real-local-browser-contract` or `real-browser-e2e`, Gate 7 execution must be `real-local-publish-candidate-contract`, `finalVerdict.publishCandidateAllowed=true`, `finalVerdict.blockingReasons=[]`, `finalVerdict.requiredGate5RealRunnerId` must match the current Gate 5 runner, `finalVerdict.requiredRealGateRunnerIds` must contain only the Gate 3, Gate 4, current Gate 5, Gate 6, and Gate 7 runner ids required for that execution path, release manifest entries must remain placeholder/not archived/not signed with `contract-accepted` signoff, rollback/share controls must defer public token creation, Gate 5 evidence must include the matching runner details (`real-browser-e2e` additionally requires `executed=true`, `playwrightExecuted=true`, `realBrowserExecuted=true`), and Gate 7 evidence must include `gate-7-real-publish-candidate-runner` details with `executed=true`, `publicShareTokenCreated=false`, and `createdPublicShareToken=null`. If any check fails, the service must downgrade Gate 7 to `failed`, set readiness/status to blocked/failed, and clear public share state.
...
- Synchronous gate runner:
  ...
  - The Gate 5 browser acceptance runner is selected by `GENERATED_APP_GATE5_EXECUTOR_MODE` or `APP_GENERATED_APP_GATE5_EXECUTOR_MODE`; supported modes are `real`, `real-browser-e2e` (also accepted as `real_browser_e2e` or `playwright`), `fixture`, and `disabled`. `GENERATED_APP_GATE5_REAL_BROWSER_UNAVAILABLE_REASON` or `APP_GENERATED_APP_GATE5_REAL_BROWSER_UNAVAILABLE_REASON` may force the real-browser E2E runner to report unavailable in a controlled environment. The persisted `generationPlan.browserAcceptancePlan.executionLevel` must be one of `browser-acceptance-skeleton | real-local-browser-contract | real-browser-e2e | fixture-browser-acceptance | disabled-browser-acceptance` and must match the configured runner level before execution.
  - The persisted `generationPlan.browserAcceptancePlan` must include stable contract-level surfaces for current AppSpec/generationPlan/staticContracts/buildUnitPlan/integrationPlan version binding, the configured execution level, a browser tool plan without real token access, desktop and mobile viewport matrix, public runtime journeys (open public runtime, fill/interact, submit, open public build preview HTML, submit through same-origin public submission APIs, wait/read result, read submission detail) bound to acceptance scenarios, requirements, public runtime API checks and static contracts, creator management journeys for generation run/gate run/submission list/detail review, console assertions, network assertions, accessibility/interaction assertions, responsive/layout assertions, screenshot/video/trace/log artifact expectations referencing Gate 4 trace artifacts, acceptance scenario coverage, requirement coverage, journey coverage, and failure capture fields. For `real-local-browser-contract`, `browserToolPlan.runner` must be `local-browser-contract`, the command must be the fixed descriptor `agentloom generated-app gate-5 local-browser-contract`, and `workingDirectory` must be generated-run relative. For `real-browser-e2e`, `browserToolPlan.runner` must be `playwright`, `runnerMode` must be `real-browser-e2e`, `serverControlled` must be true, the command must be the fixed descriptor `agentloom generated-app gate-5 real-browser-e2e`, required environment must include `GENERATED_APP_GATE5_EXECUTOR_MODE=real-browser-e2e`, allowed public endpoints must be limited to `/generated-apps/public/{token}`, forbidden endpoint patterns must include creator/internal/settings surfaces, and artifact policy must keep artifact refs under `generated-run` with host absolute paths, creator APIs, internal artifacts disabled and sensitive value redaction enabled.
  - Gate 5 checks must validate that `browserAcceptancePlan` binds the current AppSpec, generationPlan, staticContracts, buildUnitPlan, and integrationPlan versions; uses only placeholder public access values and contains no real public share token/API key/secret; defines a browser tool plan, non-empty desktop and mobile viewport coverage, public runtime journeys including `public_build_preview_submit`, creator management journeys, console/network/accessibility/responsive assertions, artifact expectations, acceptance scenario coverage, requirement coverage, journey coverage, and failure capture fields; rejects dangling requirement, scenario, static contract, Gate 4 public/creator API check, Gate 4 trace artifact, journey, viewport, assertion, and artifact references as well as illegal journey/assertion/artifact kinds; and rejects unsafe artifact paths (host absolute paths, Windows drive paths, traversal, non-`artifacts/gate-5/` paths), public journeys that reach creator/internal endpoints, creator journeys that reach public token APIs, unsafe `real-browser-e2e` runner metadata, or unsanitized console/network summary data. Gate 5 plan `passed` only means the deterministic browser acceptance plan contract is complete enough for the configured runner; it must not by itself imply a real Playwright/browser test, real screenshot/video/trace capture, real public link visit, or real end-to-end interaction executed.
  ...
  - In `real-browser-e2e` mode, Gate 5 represents the service-controlled real browser E2E runner contract for the public runtime/preview journey `open -> fill -> submit -> detail/report`. It must only use the public token runtime/preview surface (`/generated-apps/public/{token}`), must not access creator APIs, artifact APIs, internal endpoints, Workflow/plugin administration, host absolute paths, production credentials, plugin ids, workflow snapshots, raw step/checkpoint/tool data, or any secret beyond the public runtime token supplied by the server harness. If Playwright or browser binaries are unavailable, the runner must fail closed with unavailable evidence, `executed=false`, `playwrightExecuted=false`, `realBrowserExecuted=false`, and no real screenshot/video/trace capture, and must not substitute fixture or local browser contract evidence as a real E2E pass. Until a concrete Playwright adapter is wired, an availability-positive environment still fails closed rather than fabricating real E2E evidence.
  ...
  - Gate 5 gate run evidence, summary, failure message/details, and repair instructions must distinguish `browser-acceptance-skeleton`, `fixture-browser-acceptance`, `disabled-browser-acceptance`, `real-local-browser-contract`, and `real-browser-e2e`. Real-local Gate 5 evidence proves only the controlled deterministic local browser contract runner; it does not prove a Playwright run, real browser session, real screenshot/video/trace capture, real public-link visit, or full end-to-end browser execution. Real-browser E2E unavailable/not-implemented evidence proves the real E2E contract was requested and failed closed, not that a real browser journey passed.
  ...
  - In `real` mode, Gate 7 executes a service-controlled deterministic local publish-candidate contract runner. It must not execute arbitrary shell commands, user-supplied paths, production publish, artifact upload/archive creation, real signing, external verifier calls, or public share token creation. It must validate that Gate 0-6 are passed, Gate 3 execution level is `real-local-command-plan`, Gate 4 is `real-local-integration`, Gate 5 is either `real-local-browser-contract` or `real-browser-e2e`, Gate 6 is `real-local-independent-verifier`, Gate 5 evidence contains the runner id and execution flags required for the current Gate 5 execution level, the release manifest cites Gate 0-6 evidence ids, artifact refs are safe placeholders, rollback/public-share controls are deferred, no `publicShareToken` is retained or created, `finalVerdict.requiredGate5RealRunnerId` identifies the Gate 5 runner id matching the current browser execution level, `finalVerdict.requiredRealGateRunnerIds` contains only one current Gate 5 runner id, and `finalVerdict.publishCandidateAllowed=true` has no blockers. Passing Gate 7 means the app can become `publish_candidate`; it does not mean `published`, and it does not create a public token.
  ...
  - When Gate 3, Gate 4, Gate 5, or Gate 6 use fixture/disabled/skeleton execution levels, or Gate 5 `real-browser-e2e` produces unavailable/not-implemented evidence instead of trusted executed browser evidence, Gate 7 must treat that upstream gate as non-real even if its status is `passed` and must block publish candidate. When Gate 3, Gate 4, and Gate 6 are real-local, Gate 5 is real-local browser contract or real-browser E2E with matching trusted runner evidence, and the real Gate 7 runner passes, Gate 7 is recorded as `passed`, the generation run may be `passed`, readiness becomes `publish_candidate`, `publicShareToken` remains `null`, and public sharing still requires a later explicit `POST /generated-apps/:appId/public-share` readiness-guarded action.
  ...
  - After a synchronous generation run records a real passed Gate 7 result and planned generated plugin tools exist, the service must verify each generated plugin bundle before activation: resolve the Gate 3 workspace inside the configured workspace root, read the `.alp` and build report, parse and validate the manifest with the plugin SDK, recompute the archive content hash, verify the RSA-PSS signature using the generated public key recorded in the build report, require zero declared permissions, compare build-report and manifest hash/signature values, validate safe `wasmEntry` and extract the WASM bundle when present, compare build-report WASM size/hash when present, validate node definitions for the planned `toolId`, and require all generated private plugin hard gates to be present and passed. Only after those checks pass may it upload the `.alp` and WASM bundle, register a new plugin record or refresh the existing same-tenant record's archive/signature/content hash/metadata/`wasmBundleUrl` with OCC when reused, set the plugin status to `active`, and write the plugin database ids to `generated_apps.plugin_ids`. The persisted record must have `wasmBundleUrl` when WASM is present and metadata `source='generated-app-private-plugin'`, `generatedAppId`, `appSpecVersion`, `toolId`, `activationScope='tenant-private'`, Gate 3 artifact/report paths, signing verification details, WASM details, and activation hard gates.
  ...
  - Generated App runtime Workflows include a `plugin` node for each planned private plugin tool after Gate 7 activates the plugin. Each node must use the deterministic plugin id shape `com.agentloom.generated.app-<appId>.<toolId>` after lower-case segment sanitization, `pluginNodeType=<toolId>`, `pluginConfig.mode='screening'`, input ports `exec-in` and `input`, output ports `exec-out`, `analysis`, and `analysis-out`, and `smart` edges from manual trigger `exec-out -> exec-in` plus `payload-out -> input`. The graph may still keep the deterministic public report sink wired directly to the trigger payload so public runtime has a safe fallback even when the async plugin execution later fails.
  - `PluginExecutionWorker` is responsible for completing plugin workflow steps. It must run inside `runInTenantTransaction()`, set the step to `running`, execute the active tenant plugin, write `completed`/`failed` step state plus checkpoint metadata, and call `NodeSchedulerService.onNodeCompleted()` or `onNodeFailed()` so downstream workflow scheduling can progress. Normal plugins with missing `wasmBundleUrl` fail closed. Generated App tenant-private plugins with `wasmBundleUrl` must use the `PluginSandboxService` / Extism execution path. Only legacy Generated App tenant-private plugins without WASM may use the controlled deterministic no-WASM fallback, and only when metadata `source='generated-app-private-plugin'`, `activationScope='tenant-private'`, and matching `toolId` all pass; this fallback emits `analysis`/`analysis-out`, never declares external capabilities, and is not exposed as public plugin internals.
...
- Repair attempt ledger:
  ...
  - Gate 5 failed automatic repair plans must include machine-readable browser repair targets for the generated frontend runtime form, public preview HTML, public preview submission handoff, public runtime submission detail handoff, Workflow output summary mapping, and generated plugin output summary mapping. Their `e2eRunnerContract` must identify `mode='real-browser-e2e'`, command `agentloom generated-app gate-5 real-browser-e2e`, journey `open -> fill -> submit -> detail/report`, allowed `/generated-apps/public/{token}` endpoint prefix, forbidden creator/internal endpoint patterns, required redacted failure evidence, and forbidden evidence fields including public tokens, plugin ids, workflow snapshots, step/checkpoint data, raw tool data, host absolute paths, API keys, and secrets. Gate 5 reverification plans must include both local browser contract and real-browser E2E command ids, and success criteria must forbid replacing true E2E evidence with fixture or local contract evidence.
  - Failure summaries, repair instructions, and evidence summaries copied into automatic repair attempts must be redacted before persistence. Host absolute paths, traversal segments, token-like values, public token field names/values, production credentials, plugin ids, workflow snapshots, step/checkpoint data, raw tool data, source/test artifact URLs, `inputParams`, and gate-result payloads must not be preserved in `repairPlan`, `reverificationPlan`, `generationPlan.repairContext`, or repair attempt text fields.
...
- Public submissions:
  ...
  - Public/creator submission detail refresh may enrich a completed async Workflow handoff with `workflowExecutionSummary.publicOutputs`, but only from allowlisted completed step outputs: `analysis` / `analysis-out`, `text-output.content`, and `json-output.json`. The summary must sanitize forbidden keys and unsafe strings, cap the number of public outputs, truncate long text, and continue hiding raw execution steps, node data, checkpoint data, tool calls, definition snapshots, `inputParams`, plugin ids, artifact URLs, host paths, stack traces, tokens, secrets, and creator-only fields. Generated App private plugin `analysis` may appear as a business summary, but plugin identity and internal execution details must remain hidden.
...
- Good: Gate 5 real-browser-e2e unavailable evidence clearly says the service-controlled real browser E2E runner contract was requested but not executed, records the fixed command, required environment, public-only endpoint boundary, forbidden creator/internal surfaces, redacted failure evidence requirements, and false Playwright/browser/screenshot/video/trace execution flags, and never substitutes fixture or local contract evidence for a real browser pass.
...
- Good: a Gate 5 failed automatic repair attempt includes public runtime form, public preview HTML, public submission handoff, Workflow output summary, and plugin output summary patch targets plus local browser contract and real-browser E2E reverification command ids, while redacting tokens, host paths, plugin ids, workflow snapshots, step/checkpoint data, and raw tool data from the persisted work order.
...
- `generated-app.service.spec.ts`
  ...
  - synchronous runner Gate 5 real-browser-e2e failure is covered by structured `repairPlan` / `reverificationPlan` assertions for runtime form, public preview HTML, public submission handoff, Workflow/plugin output summary repair targets, local/E2E command ids, fixture/local evidence anti-substitution criteria, and redaction of host paths, tokens, plugin ids, workflow snapshots, step/checkpoint data, and raw tool data.
...
- `generated-app.browser-acceptance-runner.spec.ts`
  ...
  - Gate 5 `real-browser-e2e` fail-closed behavior records service-controlled runner contract evidence when Playwright/browser execution is unavailable, including public-only endpoint boundaries and false real execution flags, and does not present fixture/local contract evidence as a real browser pass.
  ...
  - Gate 5 runner rejects unsafe artifact paths, execution-level mismatch, public journey access to creator/internal endpoints, creator journey access to public token APIs, and real-browser E2E plans that omit `serverControlled`, public-only endpoint allowlist, forbidden creator/internal endpoint patterns, required environment, runner mode, or artifact policy before execution.

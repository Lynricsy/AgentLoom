...
- Public submissions:
  ...
  - When the Generated App has `workflowDefinitionId`, the service may create a real async Workflow execution only after resolving that Workflow in the same tenant and confirming `status='published'` plus non-null `publishedVersionId`. Draft Gate 7 editor handoff Workflows, unpublished Workflows, missing Workflows, and cross-tenant references must not call `ExecutionService.runWorkflow()`.

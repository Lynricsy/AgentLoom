...
- DB table: `generated_apps`
  ...
  - `generation_plan jsonb null`; after Gate 2 runs, the deterministic static contract skeleton is stored under `generationPlan.staticContracts`; after Gate 3 runs, the Generation Workspace contract, command plan, and build/unit execution plan are stored under `generationPlan.buildUnitPlan`; after Gate 4 runs, the deterministic integration skeleton is stored under `generationPlan.integrationPlan`; after Gate 5 runs, the deterministic browser acceptance skeleton is stored under `generationPlan.browserAcceptancePlan`; after Gate 6 runs, the deterministic independent verifier skeleton is stored under `generationPlan.independentVerificationPlan`; after Gate 7 runs, the deterministic publish-candidate guard skeleton is stored under `generationPlan.publishCandidatePlan`
...
- Synchronous gate runner:
  ...
  - If Gate 3 passes, the runner must deterministically derive `generationPlan.integrationPlan` from `AppSpec + generationPlan + staticContracts + buildUnitPlan`, persist the attempted integration plan inside `generated_apps.generation_plan`, execute the deterministic Gate 4 integration skeleton completeness check, and write one linked `generated_app_gate_runs` row for `gate-4`.
  - The persisted `generationPlan.integrationPlan` must include stable contract-level surfaces for current AppSpec/generationPlan/staticContracts/buildUnitPlan version binding, `executionLevel='integration-skeleton'`, synthetic test tenant and test resource plan without real tokens, public runtime API checks, creator management API checks, Agent/Workflow dry-run fixture expectations, plugin sandbox smoke expectations with explicit empty reason when no plugin is planned, Gate 3 dependency artifacts, acceptance scenario coverage, requirement coverage, orchestration coverage, trace artifacts, and failure capture fields.
  - Gate 4 checks must validate that `integrationPlan` binds the current AppSpec, generationPlan, staticContracts, and buildUnitPlan versions; marks `executionLevel='integration-skeleton'`; uses only synthetic test resources and contains no real public share token/API key/secret; binds public runtime API checks to current public input/output/submission persistence static contracts; models creator-side generation run/gate run/submission query checks; covers orchestration nodes/edges/input-output mappings, dry-run fixtures, plugin smoke expectations, Gate 3 build/unit artifacts, requirement/scenario/orchestration coverage, trace artifacts, and failure capture fields; and rejects dangling requirement, scenario, orchestration node, static contract, build artifact, plugin tool, API check, coverage target, and trace artifact references as well as illegal artifact/check kinds. Gate 4 `passed` only means the deterministic integration skeleton contract is complete; it must not imply a real API call, Agent/Workflow dry-run, plugin WASM/Extism smoke test, or sandbox run was executed.
  - Gate 4 gate run evidence, summary, failure message/details, and repair instructions must explicitly state that the current check is only an `integration-skeleton` / contract-level completeness check and does not prove a real public runtime API call, creator API call, Agent/Workflow dry-run, plugin WASM/Extism smoke test, or sandbox run executed.
  - If Gate 4 passes, the runner must deterministically derive `generationPlan.browserAcceptancePlan` from `AppSpec + generationPlan + staticContracts + buildUnitPlan + integrationPlan`, persist the attempted browser acceptance plan inside `generated_apps.generation_plan`, execute the deterministic Gate 5 browser acceptance skeleton completeness check, and write one linked `generated_app_gate_runs` row for `gate-5`.
  - The persisted `generationPlan.browserAcceptancePlan` must include stable contract-level surfaces for current AppSpec/generationPlan/staticContracts/buildUnitPlan/integrationPlan version binding, `executionLevel='browser-acceptance-skeleton'`, Playwright browser runner command/test entry/base URL shape without real token, desktop and mobile viewport matrix, public runtime journeys (open public runtime, fill/interact, submit, wait/read result, read submission detail) bound to acceptance scenarios, requirements, public runtime API checks and static contracts, creator management journeys for generation run/gate run/submission list/detail review, console assertions, network assertions, accessibility/interaction assertions, responsive/layout assertions, screenshot/video/trace artifact expectations referencing Gate 4 trace artifacts, acceptance scenario coverage, requirement coverage, journey coverage, and failure capture fields.
  - Gate 5 checks must validate that `browserAcceptancePlan` binds the current AppSpec, generationPlan, staticContracts, buildUnitPlan, and integrationPlan versions; marks `executionLevel='browser-acceptance-skeleton'`; uses only placeholder public access values and contains no real public share token/API key/secret; defines a browser tool plan, non-empty desktop and mobile viewport coverage, public runtime journeys, creator management journeys, console/network/accessibility/responsive assertions, artifact expectations, acceptance scenario coverage, requirement coverage, journey coverage, and failure capture fields; and rejects dangling requirement, scenario, static contract, Gate 4 public/creator API check, Gate 4 trace artifact, journey, viewport, assertion, and artifact references as well as illegal journey/assertion/artifact kinds. Gate 5 `passed` only means the deterministic browser acceptance skeleton contract is complete; it must not imply a real Playwright/browser test, real screenshot/video/trace capture, real console/network check, real public link visit, or real end-to-end interaction executed.
  - Gate 5 gate run evidence, summary, failure message/details, and repair instructions must explicitly state that the current check is only a `browser-acceptance-skeleton` / contract-level completeness check and does not prove a real Playwright/browser test, screenshot/video/trace capture, console/network inspection, public link visit, or end-to-end interaction executed.
  - If Gate 5 passes, the runner must deterministically derive `generationPlan.independentVerificationPlan` from `AppSpec + generationPlan + staticContracts + buildUnitPlan + integrationPlan + browserAcceptancePlan + Gate 0-5 gate evidence`, persist the attempted independent verifier plan inside `generated_apps.generation_plan`, execute the deterministic Gate 6 independent verifier skeleton completeness check, and write one linked `generated_app_gate_runs` row for `gate-6`.
  - The persisted `generationPlan.independentVerificationPlan` must include stable contract-level surfaces for current AppSpec/generationPlan/staticContracts/buildUnitPlan/integrationPlan/browserAcceptancePlan version binding, `executionLevel='independent-verifier-skeleton'`, verifier isolation policy declaring a fresh independent context, no generation-context reuse, no generator self-attestation, no publicShareToken/real secret reads, and redacted evidence-bundle-only inputs; evidence bundle references to Gate 0-5 gate ids/evidence ids/static contracts/build-unit artifacts/integration traces/browser artifacts/coverage matrices; rubric categories for requirement coverage, scenario coverage, UI/runtime usability, Agent/Workflow behavior, plugin/permission safety, security/privacy, data persistence, public runtime boundary, failure/error states, and publish blockers; verdict schema fields for blocking findings, warnings, pass/fail decision, traceability coverage, repair suggestions, and residual risk summary; independence checks for reviewer identity/context isolation, input redaction, generator self-attestation rejection, and evidence id citation; requirementCoverage, scenarioCoverage, evidenceCoverage, gateCoverage, and failureCaptureFields.
  - Gate 6 checks must validate that `independentVerificationPlan` binds the current AppSpec, generationPlan, staticContracts, buildUnitPlan, integrationPlan, and browserAcceptancePlan versions; marks `executionLevel='independent-verifier-skeleton'`; uses only redacted evidence bundle inputs and contains no real public share token/API key/secret; covers every required verifier isolation control, Gate 0-5 gate/evidence reference, static contract, build/unit artifact, integration trace artifact, browser artifact, coverage matrix reference, rubric category, verdict schema field/severity/decision value, independence check, requirement/scenario/evidence/gate coverage row, and failure capture field; and rejects dangling requirement, scenario, gate, evidence, static contract, build artifact, integration trace, browser artifact, rubric, verdict, and coverage references as well as illegal rubric category, verdict field, finding severity, decision value, independence check kind, coverage matrix id/source, and empty arrays. Gate 6 `passed` only means the deterministic independent verifier skeleton contract is complete; it must not imply a real independent model review, real independent agent review, real human review, real runtime-result judgment, or real requirement-satisfaction judgment executed.
  - Gate 6 gate run evidence, summary, failure message/details, and repair instructions must explicitly state that the current check is only an `independent-verifier-skeleton` / contract-level completeness check and does not prove a real independent model review, independent agent review, human review, runtime-result judgment, or requirement-satisfaction judgment executed.
  - If Gate 6 passes, the runner must deterministically derive `generationPlan.publishCandidatePlan` from `AppSpec + generationPlan + staticContracts + buildUnitPlan + integrationPlan + browserAcceptancePlan + independentVerificationPlan + Gate 0-6 gate evidence`, persist the attempted publish candidate guard plan inside `generated_apps.generation_plan`, execute the deterministic Gate 7 publish-candidate guard skeleton check, and write one linked `generated_app_gate_runs` row for `gate-7`.
  - The persisted `generationPlan.publishCandidatePlan` must include stable publish-candidate guard surfaces for current AppSpec/generationPlan/staticContracts/buildUnitPlan/integrationPlan/browserAcceptancePlan/independentVerificationPlan version binding, `executionLevel='publish-candidate-guard-skeleton'`, publish readiness inputs with Gate 0-7 gate ids, Gate 0-6 evidence ids, readiness preconditions, and required non-skeleton evidence classes, artifact release manifest entries for frontend artifact, plugin bundle artifacts, test reports, integration traces, browser artifacts, verifier report, and source artifact placeholder, publication blockers for skeleton-only upstream gates, missing real execution artifacts, missing real independent verifier verdict, unresolved warnings/blocking findings, and stale public token requirement, rollback/share controls that keep public token creation disabled while the guard fails and reference existing close/regenerate share controls, final verdict schema with `publishCandidateAllowed=false`, blocking reasons for skeleton-only upstream gates, missing real execution artifacts, missing real independent verifier verdict, warning reasons, required real gate runner ids, evidence ids, and repair suggestions, plus requirementCoverage, gateCoverage, artifactCoverage, and failureCaptureFields.
  - Gate 7 checks must validate that `publishCandidatePlan` binds the current AppSpec, generationPlan, staticContracts, buildUnitPlan, integrationPlan, browserAcceptancePlan, and independentVerificationPlan versions; marks `executionLevel='publish-candidate-guard-skeleton'`; covers all required Gate 0-7 gate ids and Gate 0-6 evidence refs; keeps required arrays non-empty; uses only allowed artifact kinds and blocker categories; rejects unknown final verdict fields; keeps `containsSecrets=false`; keeps public share creation disabled and `createdPublicShareToken=null`; covers every requirement/gate/artifact/failure capture contract; rejects dangling requirement, scenario, gate, evidence, artifact, blocker, verifier, and public-share-control references; and rejects any `publicShareToken`, API key, Bearer token, or secret leakage in the plan or evidence.
  - Gate 7 gate run evidence, summary, failure message/details, and repair instructions must explicitly state that the current check is only a `publish-candidate-guard-skeleton` and does not prove a real publish candidate was generated, real artifacts were signed off, all real quality gates passed, or public sharing can be enabled.
  - The current guard must not mark Gate 7 as `passed` while Gate 4-6 still provide only skeleton/contract-level completeness evidence rather than real integration/browser/verifier execution evidence. When Gate 3 is `fixture-execution`, `disabled-execution`, or `contract-skeleton`, Gate 7 must also treat Gate 3 as non-real upstream evidence even if Gate 3 status is `passed`.
  - If Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, Gate 5, and Gate 6 pass, Gate 7 must run and fail conservatively with a publish-candidate guard failure reason that points to the remaining skeleton-only upstream evidence; Gate 0 through Gate 6 evidence may still be recorded as `passed`, the attempted `generationPlan.publishCandidatePlan` must be retained for repair, readiness must remain non-publishable, and any stale public share token must be cleared.
  ...
  - If Gate 4 fails, the generation run must stay `failed`, `failure_reason` must describe the Gate 4 integration skeleton failure, the attempted `generationPlan.integrationPlan` must be retained for repair, Gate 5-7 must stay not passed, and any stale public share token must be cleared.
  - If Gate 5 fails, the generation run must stay `failed`, `failure_reason` must describe the Gate 5 browser acceptance skeleton failure, the attempted `generationPlan.browserAcceptancePlan` must be retained for repair, Gate 6-7 must stay not passed, and any stale public share token must be cleared.
  - If Gate 6 fails, the generation run must stay `failed`, `failure_reason` must describe the Gate 6 independent verifier skeleton failure, the attempted `generationPlan.independentVerificationPlan` must be retained for repair, Gate 7 must not run, `generationPlan.publishCandidatePlan` must not be generated or refreshed, and any stale public share token must be cleared.
...
| Synchronous runner Gate 0 passes, Gate 1 passes, Gate 2 passes, Gate 3 passes, Gate 4 passes, Gate 5 passes, and Gate 6 passes | Insert linked passed Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, Gate 5, and Gate 6 evidence, persist deterministic `generationPlan.staticContracts`, `generationPlan.buildUnitPlan`, `generationPlan.integrationPlan`, `generationPlan.browserAcceptancePlan`, and `generationPlan.independentVerificationPlan`, then persist attempted `generationPlan.publishCandidatePlan`, insert linked failed Gate 7 publish-candidate guard skeleton evidence, mark the generation run failed with the remaining skeleton-only upstream evidence failure reason, recompute readiness to non-publishable blocked/preview state, and clear any active public token |
...
| Synchronous runner Gate 5 fails                                                                                                | Insert linked passed Gate 0, Gate 1, Gate 2, Gate 3, and Gate 4 evidence plus failed Gate 5 evidence, persist the attempted `generationPlan.browserAcceptancePlan`, mark the generation run failed with the Gate 5 browser acceptance skeleton failure reason, set readiness to blocked, keep Gate 6-7 not passed, and clear any active public token                                           |
| Synchronous runner Gate 6 fails                                                                                                | Insert linked passed Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, and Gate 5 evidence plus failed Gate 6 evidence, persist the attempted `generationPlan.independentVerificationPlan`, mark the generation run failed with the Gate 6 independent verifier skeleton failure reason, set readiness to blocked, do not execute Gate 7, do not generate or refresh `generationPlan.publishCandidatePlan`, and clear any active public token                                  |
...
- Good: the synchronous runner writes linked Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, Gate 5, Gate 6, and Gate 7 evidence when earlier gates pass, persists a structured architecture `generationPlan` plus `generationPlan.staticContracts`, `generationPlan.buildUnitPlan`, `generationPlan.integrationPlan`, `generationPlan.browserAcceptancePlan`, `generationPlan.independentVerificationPlan`, and attempted `generationPlan.publishCandidatePlan`, keeps Gate 7 failed while Gate 4-6 evidence is skeleton-only, recomputes readiness to non-publishable state, and clears stale public sharing.
...
- Good: Gate 4 evidence clearly says it is a deterministic integration skeleton completeness check and not proof that a real API call, Agent/Workflow dry-run, plugin WASM/Extism smoke test, or sandbox run was executed.
- Good: Gate 5 evidence clearly says it is a deterministic browser acceptance skeleton completeness check and not proof that a real Playwright/browser test, screenshot/video/trace capture, console/network inspection, public link visit, or end-to-end interaction was executed.
- Good: Gate 6 evidence clearly says it is a deterministic independent verifier skeleton completeness check and not proof that a real independent model review, independent agent review, human review, runtime-result judgment, or requirement-satisfaction judgment was executed.
- Good: Gate 7 evidence clearly says it is a deterministic publish-candidate guard skeleton check and not proof that a real publish candidate was generated, real artifacts were signed off, all real quality gates passed, or public sharing can be enabled.
...
- Base: newly created prompt generates an AppSpec draft and can synchronously produce Gate 0 + Gate 1 + Gate 2 + Gate 3 workspace/build-unit evidence + Gate 4 + Gate 5 + Gate 6 skeleton evidence plus failed Gate 7 publish-candidate guard evidence; the app is visible to the creator but cannot create a public link.
...
- `generated-app.service.spec.ts`
  ...
  - synchronous runner Gate 0 + Gate 1 + Gate 2 + Gate 3 + Gate 4 + Gate 5 + Gate 6 pass creates linked Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, Gate 5, Gate 6, and failed Gate 7 runs, persists structured `generationPlan.staticContracts`, `generationPlan.buildUnitPlan`, `generationPlan.integrationPlan`, `generationPlan.browserAcceptancePlan`, `generationPlan.independentVerificationPlan`, and attempted `generationPlan.publishCandidatePlan`, leaves the generation run failed with a Gate 7 publish-candidate guard failure reason, clears stale public sharing, and does not produce `publish_candidate`
  ...
  - synchronous runner Gate 3 happy path stores a controlled workspace contract, command plan, real-local execution evidence, command output summaries, artifact refs, and requirement/scenario coverage while Gate 7 still blocks publish candidate because Gate 4-6 remain skeleton.
  ...
  - Gate 7 coverage treats Gate 3 `fixture-execution`, `disabled-execution`, and `contract-skeleton` evidence as non-real upstream evidence and does not allow publish candidate while Gate 4-6 remain skeleton.
  ...
  - synchronous runner Gate 5 failure is covered with a malformed but non-empty `generationPlan.browserAcceptancePlan` reference, including dangling requirement/scenario/static contract/Gate 4 API check/Gate 4 trace artifact/journey/viewport/assertion/artifact references, illegal journey/assertion/artifact kind, empty-array completeness gaps, and sensitive-token redaction assertions, so completeness checks cannot regress to "field exists means passed"
  - synchronous runner Gate 6 failure is covered with a malformed but non-empty `generationPlan.independentVerificationPlan` reference, including dangling requirement/scenario/gate/evidence/static contract/build artifact/integration trace/browser artifact/rubric/verdict/coverage references, illegal rubric category, verdict field, finding severity, decision value, independence check kind, coverage matrix id/source, empty-array completeness gaps, and sensitive-token redaction assertions, so completeness checks cannot regress to "field exists means passed"
  - synchronous runner Gate 6 failure does not execute Gate 7 and does not refresh `generationPlan.publishCandidatePlan`
  - synchronous runner Gate 7 malformed publish-candidate guard failure is covered with a malformed but non-empty `generationPlan.publishCandidatePlan` reference, including dangling requirement/scenario/gate/evidence/artifact/blocker/verifier/public-share-control references, illegal artifact kind, blocker category, verdict field, empty-array completeness gaps, and sensitive-token redaction assertions, so completeness checks cannot regress to "field exists means passed"

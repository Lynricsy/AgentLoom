...
- Synchronous gate runner:
  ...
  - The current guard must not mark Gate 7 as `passed` while Gate 5-6 still provide only skeleton/contract-level completeness evidence rather than real browser/verifier execution evidence. When Gate 4 is `fixture-integration`, `disabled-integration`, or `integration-skeleton`, Gate 7 must treat Gate 4 as non-real upstream evidence even if Gate 4 status is `passed`; when Gate 4 is `real-local-integration`, Gate 7 may stop classifying Gate 4 as skeleton-only but must still block on missing real browser/verifier artifacts and real independent verifier verdict. When Gate 3 is `fixture-execution`, `disabled-execution`, or `contract-skeleton`, Gate 7 must also treat Gate 3 as non-real upstream evidence even if Gate 3 status is `passed`.
...
- Good: the synchronous runner writes linked Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, Gate 5, Gate 6, and Gate 7 evidence when earlier gates pass, persists a structured architecture `generationPlan` plus `generationPlan.staticContracts`, `generationPlan.buildUnitPlan`, `generationPlan.integrationPlan`, `generationPlan.browserAcceptancePlan`, `generationPlan.independentVerificationPlan`, and attempted `generationPlan.publishCandidatePlan`, keeps Gate 7 failed while Gate 5-6 evidence is skeleton-only or Gate 4 is fixture/disabled/skeleton, recomputes readiness to non-publishable state, and clears stale public sharing.
...
- `generated-app.service.spec.ts`
  ...
  - synchronous runner Gate 3 + Gate 4 happy path stores a controlled workspace contract, command plan, real-local build/unit evidence, real-local Gate 4 integration trace evidence, command/output summaries, artifact refs, and requirement/scenario/staticContract coverage while Gate 7 still blocks publish candidate because Gate 5-6 remain skeleton.

...
- Public submissions:
  ...
  - Public submission must not wait for Workflow completion and must not synthesize final Workflow output. It may append only safe handoff fields to `result/report`: `workflowExecution`, `executionId`, `executionStatus`, `workflowDefinitionId`, `executionBoundary`, `workflowExecutionNotStartedReason`, and `workflowExecutionNotice`.

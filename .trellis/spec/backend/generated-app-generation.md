...
- `generationPlan.repairContext`
  ...
  - Fields: `source='previous-failed-repair-attempt'`, `sourceGenerationRunId`, `sourceRepairAttemptId`, `targetGateId`, `attemptNumber`, `status='failed'`, `failureSummary`, nullable `changeSummary`, nullable `verificationSummary`, and `capturedAt`.
...
| Retry-triggered synchronous runner has a previous failed repair attempt                                                        | Copy the latest same-app failed repair attempt into `generationPlan.repairContext`, add Gate 1 evidence `gate-1-retry-repair-context`, then continue normal Gate 1-7 execution without mutating the old attempt                                                                                                                                                                                     |
...
- Good: a retry run after a failed automatic repair attempt copies the latest failed attempt into `generationPlan.repairContext` and Gate 1 evidence so the next plan has an explicit repair target, while still saying no patch was applied until a real patch/reverification path exists.

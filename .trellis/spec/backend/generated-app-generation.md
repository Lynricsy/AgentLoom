...
- DB table: `generated_apps`
  ...
  - `generation_plan jsonb null`; after Gate 2 runs, the deterministic static contract skeleton is stored under `generationPlan.staticContracts`; after Gate 3 runs, the Generation Workspace contract, command plan, and build/unit execution plan are stored under `generationPlan.buildUnitPlan`; after Gate 4 runs, the deterministic integration plan and runner contract are stored under `generationPlan.integrationPlan`; after Gate 5 runs, the browser acceptance plan and configured runner contract are stored under `generationPlan.browserAcceptancePlan`; after Gate 6 runs, the deterministic independent verifier skeleton is stored under `generationPlan.independentVerificationPlan`; after Gate 7 runs, the deterministic publish-candidate guard skeleton is stored under `generationPlan.publishCandidatePlan`
...
- Synchronous gate runner:
  ...
  - The current guard must not mark Gate 7 as `passed` while Gate 6 still provides only skeleton/contract-level completeness evidence rather than real verifier execution evidence. When Gate 5 is `real-local-browser-contract`, Gate 7 may stop classifying Gate 5 as skeleton-only but must still block on missing real independent verifier artifacts and verdict; when Gate 5 is `fixture-browser-acceptance`, `disabled-browser-acceptance`, or `browser-acceptance-skeleton`, Gate 7 must treat Gate 5 as non-real upstream evidence even if Gate 5 status is `passed`. When Gate 4 is `fixture-integration`, `disabled-integration`, or `integration-skeleton`, Gate 7 must treat Gate 4 as non-real upstream evidence even if Gate 4 status is `passed`; when Gate 4 is `real-local-integration`, Gate 7 may stop classifying Gate 4 as skeleton-only. When Gate 3 is `fixture-execution`, `disabled-execution`, or `contract-skeleton`, Gate 7 must also treat Gate 3 as non-real upstream evidence even if Gate 3 status is `passed`.
...
- Good: the synchronous runner writes linked Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, Gate 5, Gate 6, and Gate 7 evidence when earlier gates pass, persists a structured architecture `generationPlan` plus `generationPlan.staticContracts`, `generationPlan.buildUnitPlan`, `generationPlan.integrationPlan`, `generationPlan.browserAcceptancePlan`, `generationPlan.independentVerificationPlan`, and attempted `generationPlan.publishCandidatePlan`, keeps Gate 7 failed while Gate 6 evidence is skeleton-only or Gate 5/Gate 4/Gate 3 evidence is fixture/disabled/skeleton, recomputes readiness to non-publishable state, and clears stale public sharing.
...
- Base: newly created prompt generates an AppSpec draft and can synchronously produce Gate 0 + Gate 1 + Gate 2 + Gate 3 workspace/build-unit evidence + Gate 4 integration evidence + Gate 5 browser acceptance evidence + Gate 6 skeleton evidence plus failed Gate 7 publish-candidate guard evidence; the app is visible to the creator but cannot create a public link.
...
- `generated-app.service.spec.ts`
  ...
  - synchronous runner Gate 3 + Gate 4 + Gate 5 happy path stores a controlled workspace contract, command plan, real-local build/unit evidence, real-local Gate 4 integration trace evidence, real-local Gate 5 browser contract evidence, command/output summaries, artifact refs, console/network summaries, and requirement/scenario/staticContract coverage while Gate 7 still blocks publish candidate because Gate 6 remains skeleton.

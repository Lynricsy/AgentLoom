...
- DB table: `generated_apps`
  ...
  - `generation_plan jsonb null`; after Gate 2 runs, the deterministic static contract skeleton is stored under `generationPlan.staticContracts`; after Gate 3 runs, the deterministic build/unit skeleton is stored under `generationPlan.buildUnitPlan`; after Gate 4 runs, the deterministic integration skeleton is stored under `generationPlan.integrationPlan`; after Gate 5 runs, the deterministic browser acceptance skeleton is stored under `generationPlan.browserAcceptancePlan`; after Gate 6 runs, the deterministic independent verifier skeleton is stored under `generationPlan.independentVerificationPlan`
...
- Synchronous gate runner skeleton:
  ...
  - The skeleton must not mark Gate 7 as `passed`, because it does not execute the publish-candidate gate.
  - If Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, Gate 5, and Gate 6 pass but Gate 7 is not executed, the generation run must stay conservative with `status='failed'` and `failure_reason='Gate 7 runner 尚未接入/未执行，不能形成 publish candidate。'`; Gate 0 through Gate 6 evidence may still be recorded as `passed`.
  ...
  - If Gate 6 fails, the generation run must stay `failed`, `failure_reason` must describe the Gate 6 independent verifier skeleton failure, the attempted `generationPlan.independentVerificationPlan` must be retained for repair, Gate 7 must stay not passed, and any stale public share token must be cleared.
  ...
  - When the skeleton starts from a previously publishable or published app, any unexecuted canonical Gate 7 summary must be represented as not passed for the current runner result, so the app cannot remain or become `publish_candidate` from stale evidence.
...
| Synchronous runner Gate 0 passes, Gate 1 passes, Gate 2 passes, Gate 3 passes, Gate 4 passes, Gate 5 passes, and Gate 6 passes but Gate 7 is not executed | Insert linked passed Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, Gate 5, and Gate 6 evidence, persist deterministic `generationPlan.staticContracts`, `generationPlan.buildUnitPlan`, `generationPlan.integrationPlan`, `generationPlan.browserAcceptancePlan`, and `generationPlan.independentVerificationPlan`, mark the generation run failed with the Gate 7-not-executed failure reason, keep Gate 7 not passed, recompute readiness to non-publishable preview, and clear any active public token |
...
| Synchronous runner Gate 6 fails                                                                                                | Insert linked passed Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, and Gate 5 evidence plus failed Gate 6 evidence, persist the attempted `generationPlan.independentVerificationPlan`, mark the generation run failed with the Gate 6 independent verifier skeleton failure reason, set readiness to blocked, keep Gate 7 not passed, and clear any active public token                                  |
...
- Good: the synchronous runner skeleton writes linked Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, Gate 5, and Gate 6 evidence when earlier gates pass, persists a structured architecture `generationPlan` plus `generationPlan.staticContracts`, `generationPlan.buildUnitPlan`, `generationPlan.integrationPlan`, `generationPlan.browserAcceptancePlan`, and `generationPlan.independentVerificationPlan`, leaves Gate 7 not passed until the publish-candidate runner executes, recomputes readiness to non-publishable preview, and clears stale public sharing.
...
- Base: newly created prompt generates an AppSpec draft and can synchronously produce Gate 0 + Gate 1 + Gate 2 + Gate 3 + Gate 4 + Gate 5 + Gate 6 evidence, but Gate 7 remains pending; the app is visible to the creator but cannot create a public link.
...
- `generated-app.service.spec.ts`
  ...
  - synchronous runner Gate 0 + Gate 1 + Gate 2 + Gate 3 + Gate 4 + Gate 5 + Gate 6 pass creates linked Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, Gate 5, and Gate 6 runs, persists structured `generationPlan.staticContracts`, `generationPlan.buildUnitPlan`, `generationPlan.integrationPlan`, `generationPlan.browserAcceptancePlan`, and `generationPlan.independentVerificationPlan`, leaves the generation run failed with a Gate 7-not-executed failure reason, does not mark unexecuted Gate 7 as passed, clears stale Gate 7 evidence, and does not produce `publish_candidate`

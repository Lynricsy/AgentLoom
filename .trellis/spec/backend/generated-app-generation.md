...
- Public submission detail:
  ...
  - Public submission detail may dynamically synthesize the refreshed response without writing back to `generated_app_submissions`. If it writes back, it must update only the safe handoff fields and must not overwrite `input`, token snapshots, deterministic report content, or creator audit data.

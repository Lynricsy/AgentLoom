...
- Synchronous gate runner skeleton:
  ...
  - The skeleton must not mark Gate 6-7 as `passed`, because it does not execute verifier or publish-candidate gates.
  - If Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, and Gate 5 pass but Gate 6-7 are not executed, the generation run must stay conservative with `status='failed'` and `failure_reason='Gate 6-7 runner 尚未接入/未执行，不能形成 publish candidate。'`; Gate 0 through Gate 5 evidence may still be recorded as `passed`.
  ...
  - When the skeleton starts from a previously publishable or published app, any unexecuted canonical Gate 6-7 summaries must be represented as not passed for the current runner result, so the app cannot remain or become `publish_candidate` from stale evidence.
...
| Synchronous runner Gate 0 passes, Gate 1 passes, Gate 2 passes, Gate 3 passes, Gate 4 passes, and Gate 5 passes but Gate 6-7 are not executed | Insert linked passed Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, and Gate 5 evidence, persist deterministic `generationPlan.staticContracts`, `generationPlan.buildUnitPlan`, `generationPlan.integrationPlan`, and `generationPlan.browserAcceptancePlan`, mark the generation run failed with the Gate 6-7-not-executed failure reason, keep Gate 6-7 not passed, recompute readiness to non-publishable preview, and clear any active public token |
...
- Good: the synchronous runner skeleton writes linked Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, and Gate 5 evidence when earlier gates pass, persists a structured architecture `generationPlan` plus `generationPlan.staticContracts`, `generationPlan.buildUnitPlan`, `generationPlan.integrationPlan`, and `generationPlan.browserAcceptancePlan`, leaves Gate 6-7 not passed until real runners execute them, recomputes readiness to non-publishable preview, and clears stale public sharing.
...
- Base: newly created prompt generates an AppSpec draft and can synchronously produce Gate 0 + Gate 1 + Gate 2 + Gate 3 + Gate 4 + Gate 5 evidence, but Gate 6-7 remain pending; the app is visible to the creator but cannot create a public link.
...
- `generated-app.service.spec.ts`
  ...
  - synchronous runner Gate 0 + Gate 1 + Gate 2 + Gate 3 + Gate 4 + Gate 5 pass creates linked Gate 0, Gate 1, Gate 2, Gate 3, Gate 4, and Gate 5 runs, persists structured `generationPlan.staticContracts`, `generationPlan.buildUnitPlan`, `generationPlan.integrationPlan`, and `generationPlan.browserAcceptancePlan`, leaves the generation run failed with a Gate 6-7-not-executed failure reason, does not mark unexecuted Gate 6-7 as passed, clears stale Gate 6-7 evidence, and does not produce `publish_candidate`

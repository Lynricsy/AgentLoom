...
- `generated-app.service.spec.ts`
  ...
  - synchronous runner Gate 3 + Gate 4 + Gate 5 + Gate 6 happy path stores a controlled workspace contract, command plan, real-local build/unit evidence, real-local Gate 4 integration trace evidence, real-local Gate 5 browser contract evidence, real-local Gate 6 independent verifier verdict evidence, command/output summaries, artifact refs, console/network summaries, verdict schema, and requirement/scenario/staticContract/evidence coverage while Gate 7 still blocks publish candidate because real release manifest/artifact signoff/public-share signoff is missing.

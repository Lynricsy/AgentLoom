...
describe('Organization Autonomy Policy E2E (testcontainers)', () => {
  ...
  async function seedOrganization(
    ...
    const slug = input?.slug ?? `autonomy-org-${crypto.randomUUID().slice(0, 8)}`
  ...
  it('owner can query policy summary, update policy with audit, and non-owner cannot update it', async () => {
    ...
    await seedOrganizationMember(organization.id, viewer.id, 'viewer', owner.id)
    await seedAutonomyPolicy(organization.id, organization.tenant_id, owner.id, 'RULE_BASED')
  ...
  it('confirm tightens policy, rewrites current workflow nodes, and writes confirm audits', async () => {
    ...
    expect((planner?.data.settings as Record<string, unknown>).autonomyMode).toBe(
      'MANUAL_CONFIRM',
    )
    expect((planner?.data.config as Record<string, unknown>).autonomyMode).toBe(
      'MANUAL_CONFIRM',
    )
    expect((reviewer?.data.autonomyConfig as Record<string, unknown>).mode).toBe(
      'MANUAL_CONFIRM',
    )
    expect((reviewer?.data.settings as Record<string, unknown>).autonomyMode).toBe(
      'MANUAL_CONFIRM',
    )
    expect((reviewer?.data.config as Record<string, unknown>).autonomyMode).toBe(
      'MANUAL_CONFIRM',
    )
    expect((safeNode?.data.autonomyConfig as Record<string, unknown>).mode).toBe(
      'MANUAL_CONFIRM',
    )
    expect((safeNode?.data.settings as Record<string, unknown>).autonomyMode).toBe(
      'MANUAL_CONFIRM',
    )
    expect((safeNode?.data.config as Record<string, unknown>).autonomyMode).toBe(
      'MANUAL_CONFIRM',
    )

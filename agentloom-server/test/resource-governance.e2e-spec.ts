...
async function seedExecution(options: {
  ...
  async function getExecutionStatus(executionId: string): Promise<string | null> {
    if (!ctx) {
      throw new Error('RLS test context not initialized')
    }
  ...
  it('returns the default governance state and persists quota updates with audit and notification', async () => {
    ...
    const updateResponse = await request(app!.getHttpServer())
      .put(`/api/v1/organizations/${tenant.organizationId}/resource-governance/quota`)
      .set(tenant.headers)
      .send(quotaPayload)
  ...
  it('shares the tenant minute throttle bucket across identities while isolating other tenants', async () => {
    ...
    const otherTenant = await seedTenant('resource-governance-shared-minute-other')
    ...
    const isolatedTenantResponse = await request(app!.getHttpServer())
      .get(`/api/v1/organizations/${otherTenant.organizationId}/resource-governance`)
      .set(otherTenant.headers)
  ...
  it('pauses a tenant without mutating running executions and blocks new execution starts with audit and notification', async () => {
    ...
    const controlsResponse = await request(app!.getHttpServer())
      .put(`/api/v1/organizations/${tenant.organizationId}/resource-governance/controls`)
      .set(tenant.headers)
      .send({
        tenantControl: {
          status: 'paused',
    ...
    expect(await getExecutionStatus(runningExecution.executionId)).toBe('running')
    ...
    expect(
      await countWorkflowExecutions(tenant.tenantId, workflow.workflowDefinitionId),
    ).toBe(1)

    const blockedAudits = await waitForCondition(
      () =>
        getAuditEvents(tenant.tenantId, 'resource-governance.execution-start.blocked'),
      (rows) => rows.length === 1,
    )
  ...
  it('blocks only the paused workflow while allowing sibling workflows to run', async () => {
    ...
    expect(
      controlsResponse.body.data.effectedAt,
    ).toBe(
    ...
    const blockedResponse = await request(app!.getHttpServer())
      .post(`/api/v1/workflow-definitions/${blockedWorkflow.workflowDefinitionId}/run`)
      .set(tenant.headers)
      .send({})
    ...
    const allowedResponse = await request(app!.getHttpServer())
      .post(`/api/v1/workflow-definitions/${allowedWorkflow.workflowDefinitionId}/run`)
      .set(tenant.headers)
      .send({})
  ...
  it('terminates anomalous executions with the expected action contract, audit log, and notification', async () => {
    ...
    const auditRows = await waitForCondition(
      () =>
        getAuditEvents(tenant.tenantId, 'resource-governance.execution.terminated'),
      (rows) => rows.length === 1,
    )
    expect(auditRows[0]).toMatchObject({

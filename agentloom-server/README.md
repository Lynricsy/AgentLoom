...
- **Agent 对话附件**：`POST /agent-conversations/:id/messages` 现支持 `contentType = text | image | file` 与 `metadata.attachment`；文本文件优先以内联 `resource` block 进入 runtime，上下文中的图片使用 `image` block，sandbox conversation 还会 best-effort 把附件写入 `/workspace/uploads/...` 并在 prompt 中附带工作区路径提示
...
- **Workspace 快照预览**：`WorkspaceModule` 现在除了列表/详情元数据外，还提供 `tree` / `preview` / `raw` 读取语义，支持持久化 workspace 的目录树、文本预览、图片/PDF 预览以及 unsupported 文件下载兜底

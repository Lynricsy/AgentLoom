...
| llm | `modules/llm/` | LLM 模型/提供商配置 + catalog；`llm_model_configs` 支持 `model_type`（`chat|embedding`）与可选 `embedding_dimensions`，知识库可绑定具体 embedding 模型配置；builtin provider 图标由 `provider-seeds.ts` 种到 `@lobehub/icons-static-png` 的 unpkg dark PNG URL，并对 `anthropic -> claude-color`、`google -> gemini-color`、`siliconflow -> siliconcloud-color` 等 provider/product 资产名做映射；`LlmProviderService.findAll()` / `LlmService.findAll()` 会在读取前按 sentinel 组织幂等补齐缺失的 builtin provider，并为旧迁移出的 builtin 行回填缺失或 legacy 的 `icon_url/default_base_url/base_url` 元数据 | ApiKeyModule |

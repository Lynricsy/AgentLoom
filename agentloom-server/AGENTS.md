...
| sandbox | `modules/sandbox/` | Docker 沙箱生命周期管理；workflow runtime 按 `(executionId, sandboxNodeId)` 管理 active session / persistent binding，persistent sandbox 通过 `sandbox_sessions.config.activeBindings` 追踪同一 execution 内的多节点引用；session 模式在该节点最后一个直接 consumer 完成后销毁，persistent 模式解绑后回到 `ready/idle`，显式 `stop` 与 timeout/expiry 都只会 `stop` 容器并把资源收口为 `stopped`（保留 `containerId` 供后续 `startSandbox()` 直接复用），只有 `deleteSandbox()` 才会 remove 容器并删除记录；上层 execution/agent 仍可单独失败。`SandboxModule` 通过 `SANDBOX_RUNTIME_DRIVER` token 暴露默认 `DockerService` 实现，供 agent / workspace / ACP 等消费方依赖抽象驱动；`ContainerStats` 现返回 CPU/内存以及基于 `/workspace` 文件总大小统计出的 `diskUsage`，service 侧再补 `diskTotal(bytes)` | BullMQ |
...
- **sandbox_sessions 双 FK**: `sandbox_sessions` 表使用 `execution_id` 与 `agent_conversation_id` 双 FK + CHECK 约束（至少一个非空），实现沙箱会话在工作流执行与 Agent 对话间的复用。列表 API 现会派生 `bindingType = resource | conversation | execution` 并支持同名 query filter；`SandboxConfig` 保留 `timeout(hours)` 兼容字段，同时允许 Agent runtime 使用 `timeoutSeconds` 做秒级 timeout 调度。`agent_definitions.sandboxConfig` 与 `agent_versions.snapshot.sandboxConfig` 现在被视为 persisted mirror：detail 响应、版本响应、standalone Agent runtime 与 workflow-agent runtime 都会优先根据 canvas / snapshot 的 `nodes + edges` 重新推导 canonical sandboxConfig，用于恢复旧记录里缺失的 `timeoutSeconds` / `restoreWorkspaceId`。

...
- **Story 8-6 已完成自动化收口**: canonical `WorkflowInputSchema` 现扩展最小 form-baseline visibility DSL `visibility: { fieldId, equals }`（`equals` 收紧为 `string | number`，且 `fieldId` 必须引用已有字段）；`GET/PATCH /workflow-definitions/:id` detail/update 现支持 draft `inputSchema` hydrate 与持久化；`inputSchema.version` 只在逻辑 schema diff 时递增，继续独立于 workflow OCC `version`。`POST /workflow-definitions/:id/run` 现接受 `schemaVersion` / `schema_version`，`ExecutionService` 会基于 published schema 做 required/default/visibility/unknown-field 校验，并把规范化结果写入 `workflow_executions.input_params._meta.launchConfig { workflowId, schemaVersion, collectionMode, resolvedInputs, unresolvedFieldIds, launchSource }`，同时保留既有 `_meta.launchSource` 与顶层 `inputParams` 兼容路径；`WorkflowLaunchSchemaVersionMismatchException` 返回 409，`WorkflowLaunchInputValidationException` 返回 422。Story 8-6a 的服务端 S1/S2 进一步把 canonical DTO 扩展为支持字段级 `collectionHint?: string` 与可选 `conversationPlan { systemPrompt, maxTurns }`，且 `conversation` / `hybrid` 不再走 legacy fallback，而是和 `form` 一样统一经过 schemaVersion、required/default、visibility 与 canonical `launchConfig` 归一化链路。

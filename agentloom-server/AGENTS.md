...
- **Story 6-3 / Evidence Query Filters**: `QueryEvidenceSchema` 新增 `sourceType` (EvidenceSourceType enum) 和 `nodeId` (string, min 1) 可选过滤参数。`findByExecution()` 支持按 `sourceType` 直接 `eq()` 过滤，按 `nodeId` 先查 `execution_steps` 获取匹配的 step IDs 再 `inArray()` 过滤。Controller 无需改动，直接透传 query 参数。

...
| knowledge | `modules/knowledge/` | RAG: 解析 → 分块 → Qdrant 向量索引；`VECTOR_STORE` token 以 `useClass: QdrantVectorStoreService` 注入；`knowledge_bases.embedding_model_config_id` 可绑定具体 embedding 模型配置，优先于 legacy `embedding_model`；`parsers/` 含 4 个解析器 + dispatcher | BullMQ, Qdrant |

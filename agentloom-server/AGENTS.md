...
| agent | `modules/agent/` | **六边形架构**: ports/AgentRuntime → InProcess\|Sandbox 适配器；`PiAgentCoreAdapter` 封装 pi-agent-core `Agent` 运行时，`InProcessAgentAdapter` 作为持久化包装层委托 runtime 操作；`SandboxAgentAdapter` 通过 HTTP POST + SSE 流与 `agentloom/sandbox:latest` 容器通信；`tool-schema-converter.ts` 处理 Zod ↔ TypeBox 双向转换；`pi-imports.ts` 提供 ESM 动态导入入口；`stream-fn.adapter.ts` 桥接 Vercel AI SDK `streamText()` 与 pi-agent-core `StreamFn` 接口 | LlmModule, SandboxModule |

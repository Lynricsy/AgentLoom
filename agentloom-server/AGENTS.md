...
| agent-conversation | `modules/agent-conversation/` | Agent 对话生命周期管理：创建/列表/消息历史/发送消息 API、工具权限 resolve、`GET /agent-conversations/:id/sandbox/stats` 与 `POST /agent-conversations/:id/restart-latest-version`；`agent_conversations`/`agent_messages` 表 | AgentDefinitionModule |

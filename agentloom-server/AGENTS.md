...
- **sandbox_sessions 双 FK**: `sandbox_sessions` 表使用 `execution_id` 与 `agent_conversation_id` ��� FK + CHECK 约束（至少一个非空），实现沙箱会话在工作流执行与 Agent 对话间的复用。列表 API 现会派生 `bindingType = resource | conversation | execution` 并支持同名 query filter；`SandboxConfig` 保留 `timeout(hours)` 兼容字段，同时允许 Agent runtime 使用 `timeoutSeconds` 做秒级 timeout 调度。

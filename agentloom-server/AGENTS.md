...
| sandbox | `modules/sandbox/` | Docker 沙箱生命周期管理；workflow runtime 按 `(executionId, sandboxNodeId)` 管理 active session / persistent binding，persistent sandbox 通过 `sandbox_sessions.config.activeBindings` 追踪同一 execution 内的多节点引用；session 模式在该节点最后一个直接 consumer 完成后销毁，persistent 模式解绑后回到 `ready/idle`，显式 `stop` 与 timeout/expiry 都只会 `stop` 容器并把资源收口为 `stopped`（保留 `containerId` 供后续 `startSandbox()` 直接复用），只有 `deleteSandbox()` 才会 remove 容器并删除记录；direct Agent conversation 的 sandbox config 额外支持 `conversationIdleAutoEndMinutes`，当同一 sandbox 下所有 active conversation 都无运行中 loop 且无未处理 user message 时，会由 sandbox lifecycle delayed job 自动调用 `end()` 收口 conversation，再沿用既有 ended 链路释放/解绑沙箱。`SandboxModule` 通过 `SANDBOX_RUNTIME_DRIVER` token 暴露默认 `DockerService` 实现，供 agent / workspace / ACP 等消费方依赖抽象驱动；`ContainerStats` 现返回 CPU/内存以及基于 `/workspace` 文件总大小统计出的 `diskUsage`，service 侧再补 `diskTotal(bytes)` | BullMQ |
...
| agent-definition | `modules/agent-definition/` | Agent 定义 CRUD + 版本管理 + canvas 保存 + 发布/归档；`agent_definitions`/`agent_versions` 表；runtime config 接口定义 Agent 运行参数（CPU/memory/timeout/lifecycle）并扩展 `nativeToolPolicy/selfEvolutionPolicy` | WorkspaceModule |
| agent-conversation | `modules/agent-conversation/` | Agent 对话生命周期管理：创建/列表/消息历史/发送消息 API、工具权限 resolve、`GET /agent-conversations/:id/sandbox/stats` 与 `POST /agent-conversations/:id/restart-latest-version`；`ConversationTitleService` 会在首轮 assistant 回复后自动生成标题，并在标题模型不可用或 LLM 生成失败时回退为首条用户消息摘要；`agent_conversations`/`agent_messages` 表 | AgentDefinitionModule |
| agent-execution | `modules/agent-execution/` | Agent 对话执行引擎：`AgentExecutionWorker` 处理对话消息、`AgentConversationGateway`（Socket.IO `/agent-conversation` namespace）实时推送、`WorkspaceIntegrationService` 文件集成、`WorkflowAgentAdapter` 桥接工作流 `agent` 节点执行；`spawn_subagent` 后台执行会脱离父工具回调事务并在新的 tenant transaction 中运行，避免复用已结束的 callback 事务 | AgentModule, SandboxModule, Socket.IO |
...
- **Workflow 版本语义**: `workflow_definitions.version` 继续只承担草稿 OCC/修订号；用户可见的发布版号写在 `workflow_versions.snapshot.metadata.releaseNumber`，仅在快照首次发布时分配。`WorkflowVersionService` 列表/详情响应会额外暴露 `publishedReleaseNumber`，版本历史中的 `versionNumber` 现在应被理解为“快照号”，不是用户可见发布版本号。

...
| sandbox | `modules/sandbox/` | Docker 沙箱生命周期管理；workflow runtime 按 `(executionId, sandboxNodeId)` 管理 active session / persistent binding，persistent sandbox 通过 `sandbox_sessions.config.activeBindings` 追踪同一 execution 内的多节点引用；session 模式在该节点最后一个直接 consumer 完成后销毁，persistent 模式解绑后回到 `ready/idle`，命中 timeout/expiry 时资源本体也收口为 `stopped`（上层 execution/agent 可单独失败）；`SandboxModule` 通过 `SANDBOX_RUNTIME_DRIVER` token 暴露默认 `DockerService` 实现，供 agent / workspace / ACP 等消费方依赖抽象驱动；`ContainerStats` 现返回 CPU/内存以及基于 `/workspace` 文件总大小统计出的 `diskUsage`，service 侧再补 `diskTotal(bytes)` | BullMQ |

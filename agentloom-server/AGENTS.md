...
| sandbox | `modules/sandbox/` | Docker 沙箱生命周期管理；workflow runtime 按 `(executionId, sandboxNodeId)` 管理 active session / persistent binding，persistent sandbox 通过 `sandbox_sessions.config.activeBindings` 追踪同一 execution 内的多节点引用；session 模式在该节点最后一个直接 consumer 完成后销毁，persistent 模式解绑后回到 `ready/idle` | BullMQ |

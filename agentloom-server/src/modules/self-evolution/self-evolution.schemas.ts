...
import type { ReactFlowEdge, ReactFlowNode } from '../../database/schema/workflow-definitions.schema';
...
export const QueryResourcePoolSchema = z.object({
  resourceType: z.enum(['skill', 'mcp_server', 'mcp_tool', 'model', 'agent', 'workflow', 'workspace']).optional(),
...
export const ApplyChangeSchema = z.object({ proposal: GenericRecordSchema }).strict();
export const CreateResourceSchema = z.object({
  resourceType: z.enum(['skill', 'workspace', 'agent', 'workflow', 'mcp', 'model']),
  spec: GenericRecordSchema,
}).strict();

export const SkillCreateDtoSchema = z.object({
  name: z.string().min(1),
  description: z.string(),
  content: z.string().optional(),
}).strict();

const AgentGraphNodeSchema = WorkflowGraphNodeSchema.partial({ position: true }).extend({
  data: GenericRecordSchema.optional(),
});
export const AgentGraphNodeArraySchema = z.array(AgentGraphNodeSchema).pipe(z.custom<ReactFlowNode[]>());
export const AgentGraphEdgeArraySchema = z.array(WorkflowGraphEdgeSchema).pipe(z.custom<ReactFlowEdge[]>());
export const AgentCreateDtoSchema = z.object({
  name: z.string().min(1),
  description: z.string().optional(),
  icon: z.string().optional(),
}).strict().pipe(z.custom<CreateAgentDefinitionDto>());
...
export const McpImportDtoSchema = z.object({
...
}).strict().pipe(z.custom<ImportMcpToolsDto>());

export const ProviderCreateDtoSchema = z.object({
  name: z.string().min(1),
...
}).strict().pipe(z.custom<CreateLlmProviderDto>());
...
export const ModelCreateDtoSchema = z.object({
  name: z.string().min(1),
...
}).strict().pipe(z.custom<CreateLlmModelConfigDto>());

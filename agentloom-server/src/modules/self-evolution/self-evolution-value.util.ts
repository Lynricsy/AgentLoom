...
export function readRequiredRecord(value: unknown, fieldName: string): EvolutionRecord {
  const normalized = readRecord(value);
  if (!normalized) throw new Error(`${fieldName} 必须是对象`);
  return normalized;
}
export function readRequiredStringArray(value: unknown, fieldName: string): string[] {
  const normalized = readStringArray(value);
  if (normalized.length === 0) throw new Error(`${fieldName} 至少需要包含一个字符串`);
  return normalized;
}
...
export function cloneJsonRecord(value: unknown): EvolutionRecord {
  return readRecord(value) ? JSON.parse(JSON.stringify(value)) as EvolutionRecord : {};
}
...
export function readTargetKind(value: unknown): SelfEvolutionTargetKind {
  if (value === 'self' || value === 'agent' || value === 'workflow') return value;
  throw new Error('targetKind 必须是 self / agent / workflow');
}
function readGraphOperation(value: unknown, fieldName: string): 'add' | 'update' | 'remove' {
...
export function readNodeOperations(value: unknown): GraphNodeOperation[] {
  ...
  return value.map((entry) => {
    ...
    return {
      ...
      ...(readString(record.nodeId) ? { nodeId: readString(record.nodeId) } : {}),
...
export function readEdgeOperations(value: unknown): GraphEdgeOperation[] {
  ...
  return value.map((entry) => {
    ...
    return {
      ...
      ...(readString(record.edgeId) ? { edgeId: readString(record.edgeId) } : {}),
      ...(readRecord(record.edge) ? { edge: readRecord(record.edge)! } : {}),
      ...(readRecord(record.patch) ? { patch: readRecord(record.patch)! } : {}),
    };
  });
}
export function readProposal(value: unknown): SelfEvolutionGraphProposal | null {
  const record = readRecord(value);
  if (!record || record.domain !== SELF_EVOLUTION_DOMAIN) return null;
  const category = readString(record.category);
  if (!category || !(SELF_EVOLUTION_CATEGORY_VALUES as readonly string[]).includes(category)) return null;
  const targetKind = readString(record.targetKind);
  if (targetKind !== 'self' && targetKind !== 'agent' && targetKind !== 'workflow') return null;
  return {
    ...
    baseVersion: readPositiveInt(record.baseVersion, 1, Number.MAX_SAFE_INTEGER),
    ...
    ...(readRecord(record.viewport) ? { viewport: readRecord(record.viewport)! } : {}),
    ...(readRecord(record.metadataPatch) ? { metadataPatch: readRecord(record.metadataPatch)! } : {}),
    summary: readRequiredString(record.summary, 'proposal.summary'),
    category: category as SelfEvolutionCategory,
    riskLevel: record.riskLevel === 'low' || record.riskLevel === 'high' ? record.riskLevel : 'medium',
    requiresConfirmation: Boolean(record.requiresConfirmation),
    diffPreview: readRecord(record.diffPreview) ?? { summary: 'No diff preview' },
  };
}
export function buildSkillFiles(filesValue: unknown, contentValue: unknown): SkillUploadFile[] | undefined {
  const files = readRecord(filesValue);
  const entries = files ? Object.entries(files).filter((entry): entry is [string, string] => entry[0].length > 0 && typeof entry[1] === 'string') : [];
  if (entries.length === 0 && typeof contentValue !== 'string') return undefined;
  const normalized = entries.length > 0 ? entries : [['SKILL.md', String(contentValue ?? '')] as const];
  return normalized.map(([filename, content]) => ({ fieldname: 'files', filename, buffer: Buffer.from(content, 'utf-8'), mimetype: 'text/markdown' }));
}
export function hasNewPublishedVersion(previous: string | null | undefined, next: string | null | undefined): next is string {

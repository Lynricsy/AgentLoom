...
describe('ApiKeyController', () => {
  ...
  describe('rotate', () => {
    it('应当调用 service.rotate 并包装返回值', async () => {
      ...
      const result = await controller.rotate(KEY_ID, dto, TENANT_ID);

      expect(result).toEqual({ data: MOCK_RESPONSE });
      expect(service.rotate).toHaveBeenCalledWith(KEY_ID, dto, TENANT_ID);
    });
  });

  describe('revoke', () => {
    it('应当调用 service.revoke 并包装返回值', async () => {
      const result = await controller.revoke(KEY_ID, TENANT_ID);

      expect(result).toEqual({ data: { ...MOCK_RESPONSE, status: 'revoked' } });
      expect(service.revoke).toHaveBeenCalledWith(KEY_ID, TENANT_ID);
    });
  });
});

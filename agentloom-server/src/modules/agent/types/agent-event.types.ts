...
export type AgentEvent = PlanEvent | MessageChunkEvent | ToolCallAgentEvent | DoneEvent

export function isPlanEvent(event: AgentEvent): event is PlanEvent {
  return event.type === 'plan'
}

export function isMessageChunkEvent(event: AgentEvent): event is MessageChunkEvent {
  return event.type === 'message_chunk'
}

export function isToolCallEvent(event: AgentEvent): event is ToolCallAgentEvent {

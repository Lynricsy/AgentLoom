...
describe('ContentBlock Zod Schema', () => {
  ...
  describe('ImageContentBlockSchema', () => {
    ...
    it('应拒绝非 image/ 开头的 mimeType', () => {
      ...
      expect(() => ImageContentBlockSchema.parse(input)).toThrow(
  ...
  describe('AudioContentBlockSchema', () => {
    ...
    it('应拒绝非 audio/ 开头的 mimeType', () => {
      ...
      expect(() => AudioContentBlockSchema.parse(input)).toThrow(
        'mimeType 必须以 audio/ 开头',
      );
    });
  });
  ...
  describe('ResourceContentBlockSchema', () => {
    ...
    it('应拒绝空 URI', () => {
      ...
      expect(() => ResourceContentBlockSchema.parse(input)).toThrow(
        'URI 不能为空',
      );
    });
  });
  ...
  describe('ContentBlockSchema (判别联合)', () => {
    it('应根据 type 字段正确区分各变体', () => {
      expect(ContentBlockSchema.parse({ type: 'text', text: 'hi' }).type).toBe(
        'text',
      );
      expect(
      ...
      expect(
        ContentBlockSchema.parse({ type: 'resource', uri: 'u' }).type,
      ).toBe('resource');
      expect(
        ContentBlockSchema.parse({ type: 'resource_link', uri: 'u' }).type,
      ).toBe('resource_link');
    });
  ...
  describe('ContentBlockArraySchema', () => {
    ...
    it('应拒绝空数组', () => {
      expect(() => ContentBlockArraySchema.parse([])).toThrow(
        '至少需要一个 ContentBlock',
      );
    });
  });

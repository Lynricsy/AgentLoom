...
describe('IAgentRuntime 接口契约', () => {
  ...
  function createMockRuntime(): IAgentRuntime {
    return {
      createSession: vi.fn(async (params: CreateSessionParams) =>
        createMockSession(params),
      ),
      loadSession: vi.fn(async (_sessionId: string) =>
        createMockSession({
  ...
  describe('createSession', () => {
    ...
    it('应兼容 MCP 模块的 streamable_http 传输类型', async () => {
      ...
      expect(session.context.mcpServers?.docs?.transportType).toBe(
        'streamable_http',
      );
      expect(session.context.mcpServers?.docs?.url).toBe(
        'https://example.com/mcp',
      );
      expect(session.context.mcpServers?.docs?.headers?.Authorization).toBe(
        'Bearer token',
      );
    });

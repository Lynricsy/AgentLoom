...
export class GeneratedAppGate3WorkspaceRunner {
  ...
  private buildGate3PluginBuildScript(): string {
    return [
      ...
      '  }',
      '  if (typeof pluginManifest.wasmEntry !== "string" || pluginManifest.wasmEntry !== "dist/plugin.wasm") issues.push("wasmEntry must be dist/plugin.wasm");',
      ...
      '  return [...uleb(buffer.length), ...buffer];',
      '}',
      'function wasmSection(id, bytes) {',
      '  return [id, ...uleb(bytes.length), ...bytes];',
      '}',
      'function wasmFuncType(params, results = []) {',
      '  return [0x60, ...uleb(params.length), ...params, ...uleb(results.length), ...results];',
      '}',
      'function createGeneratedPrivatePluginWasm() {',
      '  const valueType = { i32: 0x7f, i64: 0x7e };',
      '  const response = {',
      '    analysis: {',
      '      riskLevel: "follow-up",',
      '      score: 42,',
      '      signalCount: 1,',
      '      followUpQuestions: ["请补充症状持续时间、诱因和缓解因素。"],',
      '      boundaryNotice: "本工具只做信息整理和追问优先级提示，不提供诊断、处方、剂量或治疗指令。",',
      '      generatedPrivatePlugin: true,',
      '      runtime: "wasm-extism"',
      '    }',
      '  };',
      '  response["analysis-out"] = response.analysis;',
      '  const outputBytes = Buffer.from(JSON.stringify(response), "utf8");',
      '  const types = [',
      '    wasmFuncType([valueType.i64], [valueType.i64]),',
      '    wasmFuncType([valueType.i64, valueType.i32]),',
      '    wasmFuncType([valueType.i64, valueType.i64]),',
      '    wasmFuncType([]),',
      '  ];',
      '  const imports = [',
      '    [...wasmString("extism:host/env"), ...wasmString("alloc"), 0x00, ...uleb(0)],',
      '    [...wasmString("extism:host/env"), ...wasmString("store_u8"), 0x00, ...uleb(1)],',
      '    [...wasmString("extism:host/env"), ...wasmString("output_set"), 0x00, ...uleb(2)],',
      '  ];',
      '  const body = [];',
      '  body.push(0x42, ...sleb(outputBytes.length));',
      '  body.push(0x10, ...uleb(0));',
      '  body.push(0x21, ...uleb(0));',
      '  for (let index = 0; index < outputBytes.length; index += 1) {',
      '    body.push(0x20, ...uleb(0));',
      '    body.push(0x42, ...sleb(index));',
      '    body.push(0x7c);',
      '    body.push(0x41, ...sleb(outputBytes[index]));',
      '    body.push(0x10, ...uleb(1));',
      '  }',
      '  body.push(0x20, ...uleb(0));',
      '  body.push(0x42, ...sleb(outputBytes.length));',
      '  body.push(0x10, ...uleb(2));',
      '  body.push(0x0b);',
      '  const locals = [...uleb(1), ...uleb(1), valueType.i64];',
      '  const funcBody = [...uleb(locals.length + body.length), ...locals, ...body];',
      '  return Buffer.from([',
      '    0x00, 0x61, 0x73, 0x6d,',
      '    0x01, 0x00, 0x00, 0x00,',
      '    ...wasmSection(1, [...uleb(types.length), ...types.flat()]),',
      '    ...wasmSection(2, [...uleb(imports.length), ...imports.flat()]),',
      '    ...wasmSection(3, [...uleb(1), ...uleb(3)]),',
      '    ...wasmSection(7, [...uleb(1), ...wasmString("execute"), 0x00, ...uleb(3)]),',
      '    ...wasmSection(10, [...uleb(1), ...funcBody]),',
      '  ]);',
      '}',
      ...
      '  const wasmBundle = createGeneratedPrivatePluginWasm();',
      '  const unsignedEntries = [',
      '    { name: "node-definitions.json", data: `${JSON.stringify(nodeDefinitions, null, 2)}\\n` },',
      '    { name: pluginManifest.wasmEntry, data: wasmBundle },',
      ...
      '    wasmEntry: pluginManifest.wasmEntry,',
      '    wasmRuntime: "extism",',
      '    wasmSizeBytes: wasmBundle.length,',
      '    wasmSha256: sha256Hex(wasmBundle),',
      '    manifestValid: true,',

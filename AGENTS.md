...
- **Story 8-4 已完成**: 多智能体层级溯源图。Server: 新增 `EvidenceGraphService.buildGraph(tenantId, executionId)` 查询 `execution_steps` 建立 stepId→nodeId 映射，聚合 `evidence_records` 按 nodeId 生成图节点（含 evidenceCount/firstEvidenceAt/lastEvidenceAt），通过 `parentEvidenceId` 跨节点链接推导图边（含 evidenceLinks 计数/dataTypeSummary），`buildTimeline()` 生成时间线条目（节点首次出现 + 首条跨节点边）；Redis 缓存 300s（key `evidence:graph:{executionId}`），空证据返回空图不缓存。`GET /executions/:executionId/evidence/graph`（VIEWER+，X-Cache-Hit header）。DTO: `AgentGraphNodeSchema`/`AgentGraphEdgeSchema`/`GraphTimelineEntrySchema`/`EvidenceGraphResponseSchema`。17 个单测覆盖串行/并行/空/单节点/时间线/缓存/钻石拓扑场景。Studio: `features/evidence/` 新增 4 个 graph 类型（`AgentGraphNode`/`AgentGraphEdge`/`GraphTimelineEntry`/`EvidenceGraphResponse`）；`fetchEvidenceGraph` + `evidenceKeys.graph` + `useEvidenceGraph`（staleTime 5min）；`evidenceUiStore` 新增 `graphSelectedNodeId` + `setGraphSelectedNodeId`；5 个组件：`AgentGraphNode.tsx`（React Flow 自定义节点，6 种节点类型图标 + 6 种状态颜色 + 证据徽章 + Handle）、`AgentGraphEdge.tsx`（自定义边，高亮时 tooltip + dash-flow 动画）、`GraphTimelinePlayer.tsx`（播放/暂停/重置，800ms 间隔，内部步进状态）、`EvidenceGraphControls.tsx`（dagre/力导向布局切换 + fitView + 刷新）、`EvidenceGraphView.tsx`（主容器：dagre rankdir:TB nodesep:80 ranksep:120 + d3-force 布局，React Flow minZoom:0.3 maxZoom:2，时间线高亮，节点点击→openPanel）；`ExecutionDebugView` 新增 "调试面板"/"溯源图" 双 tab 切换（shared `Tabs` 组件）；21 个组件单测。Studio 依赖新增 `@dagrejs/dagre 2.0.4`、`d3-force 3.0.0`、`@types/d3-force 3.0.10`。Server `pnpm test` 1293 passed、`pnpm build` 通过。Studio `pnpm typecheck` 通过、`pnpm test` 1032 passed、`pnpm build` 通过。

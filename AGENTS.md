...
- **Story 8-3 Server T4-T10 已完成**: 后端新增 `TriggerModule` 与 `workflow_triggers/workflow_trigger_history` 执行链路收口。Server: `src/modules/trigger/` 现包含 `TriggerService`、`TriggerHistoryService`、`TriggerSchedulerService`、`TriggerSchedulerProcessor`、`WebhookService`、`WebhookController`、`TriggerController`、`GithubWebhookAdapter`、`trigger.module.ts` 与完整 unit tests；`main.ts` 现以 `NestFactory.create(..., { rawBody: true })` 启用 webhook 原始请求体；`AppModule.configure()` 显式排除了 `webhooks` / `webhooks/{*splat}` 以放行公共 webhook 路由。功能上已支持 workflow trigger CRUD、cron repeatable jobs 同步/移除、webhook HMAC-SHA256 验签与 IP 白名单、trigger history 记录、启停切换及 cron init sync。由于预置 `trigger.dto.ts` 使用了当前 zod 运行时不提供的 `z.string().ip()`，本轮以 `zod-ip.polyfill.ts` + `trigger-dto.compat.ts` + `src/types/zod-ip-compat.d.ts` 做兼容而未直接修改 T1-T3 预置文件。验证结果：Server `pnpm test` 1265 passed，`pnpm build` 通过。

...
| 管理服务端插件注册与��态 | `agentloom-server/src/modules/plugin/` | `.alp` multipart 注册 + canonical archive RSA-PSS 验签、`plugins`/`plugin_developer_keys` 表、`PluginSandboxService`（Extism WASM 沙箱）、开发者密钥管理 API、`plugin-execution` 队列 |
...
- **PluginModule**: `plugins` 表保存租户插件元数据（`org_id + plugin_id` 唯一，含 `manifest`、`node_definitions`、`signature`、`content_hash`、`wasm_bundle_url`、`occ_version` 与 tenant RLS）。`plugin_developer_keys` 表管理开发者 RSA 公钥（`org_id + key_fingerprint` 唯一，active/revoked 状态）。`/plugins` 提供 `.alp` multipart 注册（强制要求 `signature/contentHash/developerKeyFingerprint`，按 canonical unsigned archive payload 做 RSA-PSS 验签，通过后才上传归档与提取 WASM）、列表、详情、状态更新与删除；`/plugins/developer-keys` 提供开发者密钥注册、列表、详情与撤销；`PluginSignatureService` 复用 `@agentloom/plugin-sdk` 的 canonical archive helper 计算内容哈希与验签；`PluginSandboxService` 使用 `@extism/extism` 创建隔离 WASM 实例（`runInWorker: true`），平台硬限制固定 `timeoutMs=30000` / `maxMemoryPages=4096`，manifest 仅可在声明 `network:outbound` 时提供 `sandbox.allowedHosts` 白名单，runtime config 只能进一步收紧这些限制；`PluginExecutionWorker` 从 MinIO 下载 WASM → 构建并收紧沙箱配置 → 默认调用导出函数 `execute`

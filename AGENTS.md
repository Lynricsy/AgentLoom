...
- **沙箱容器运行时 (pi-coding-agent)**: `agentloom/sandbox:latest` 基于 archlinux 镜像，内嵌 `pi-coding-agent` + Fastify HTTP 适配层。容器暴露 `POST /v1/session`（创建会话）、`POST /v1/prompt`（SSE 流式应答，兼容 JSON-RPC 2.0 envelope 与直接 AgentEvent 两种格式）、`POST /v1/abort`（取消）、`GET /health`（健康检查）。LLM API Key 通过环境变量注入。standalone Agent 对话与 `WorkflowAgentAdapter` 都走 sandbox runtime，但 standalone Agent 对话现在要求画布中存在连接到 `agent-main.sandbox-in` 的 sandbox 节点；若未连接任何沙箱，服务端会显式返回配置错误，而不是隐式合成默认 sandbox。`PiConfigGeneratorService` 从 Agent 定义生成 pi 配置文件挂载到容器 `/config/`

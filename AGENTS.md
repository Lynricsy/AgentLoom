...
- **Story 2-1a 已完成**: 工作流 CRUD、查询与自动保存 API。Server: `PATCH /workflow-definitions/:id` 自动保存端点（OCC 乐观并发控制，version 字段 WHERE 条件 + 自增），`DELETE /workflow-definitions/:id` 软删除（委托 archive）；`GET /workflow-definitions/:id` 现返回含 nodes/edges/viewport 完整详情；列表查询支持 sort (updatedAt/createdAt/name) + order (asc/desc)；`UpdateWorkflowDefinitionDto` 使用 Zod `.strict()` + version 必填；`WorkflowVersionConflictException` (409) 含 currentVersion；`AllExceptionsFilter` 现防御性处理 ZodValidationException（修复 Zod v4 `z.record()` 单参数 bug 导致的 500）。96 单元 + 33 E2E 测试全部通过

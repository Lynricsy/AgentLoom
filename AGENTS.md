...
| 管理 Agent 对话 | `agentloom-server/src/modules/agent-conversation/` + `agentloom-studio/src/features/agent-conversation/` + `agentloom_mobile/lib/features/agents/` | 对话生命周期 CRUD、消息历史 API、工具权限 resolve / restart-latest-version API、`agent_conversations`/`agent_messages` 表、Studio 三列对话 UI、Mobile 对话屏 |
...
- **Workflow Sandbox 语义**: workflow runtime 以 `(execution_id, sandboxNodeId)` 作为 sandbox session / persistent binding 粒度；persistent sandbox 通过 `sandbox_sessions.config.activeBindings` 追踪同一 execution 内的多节点引用。一个 sandbox 节点可以同时供多个下游 agent 共享；多个 sandbox 节点也可以引用同一 persistent sandbox 资源，但生命周期按节点独立释放。session 模式在该节点最后一个直接 consumer 完成后销毁，persistent 模式解绑后回到 `ready/idle`；对 persistent 资源，显式 `stop` 与 timeout/expiry 只会停止容器并把状态收口为 `stopped`，保留既有 `containerId` 供后续 `startSandbox()` 复用，只有手动 `deleteSandbox()` 才会 remove 容器并删除记录
...
- **Agent 双运行态**: Agent 创建时显式持久化 `runtimeMode = sandbox | no_sandbox`，创建后固定。顶层 standalone / workflow `agent` 节点中的 `no_sandbox` Agent 走 `InProcessAgentAdapter -> PiAgentCoreAdapter -> pi-agent-core`，仍支持 Skill、知识库、Memory、HTTP MCP 与自进化；顶层 `sandbox` Agent 继续走 `agentloom/sandbox:latest`（`pi-coding-agent` + Fastify HTTP）容器 runtime。`no_sandbox` Agent 不暴露内置 coding tools，且只允许 HTTP MCP（stdio MCP 在发布校验与运行时调用两侧都被拒绝）；当 `sandbox` Agent 调用 `no_sandbox` 子 Agent 时，child 会并入父 sandbox runtime 配置，并收敛为只开放 `read` 的只读内置工具语义。`PiConfigGeneratorService` 继续负责从 Agent 定义生成 sandbox runtime 所需配置文件并挂载到容器 `/config/`

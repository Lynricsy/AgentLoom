...
- **Story 6-4 已完成**: 证据引用面板与文档精准跳转。Server: `StorageService.getPresignedUrl()` 生成 MinIO 预签名 URL；`DocumentService.getDocumentContentUrl()` 返回 `{url, fileName, mimeType, expiresIn}`；`KnowledgeBaseController GET :kbId/documents/:docId/content` 端点（VIEWER+）；`QueryEvidenceSchema.includeChunkContent` 布尔参数，`EvidenceService.enrichWithChunkContent()` 批量注入 `rag_retrieval` 记录的 `documentChunks.content`。Studio: `evidenceUiStore`（Zustand）管理面板/文档查看器状态（openPanel/selectEvidence/openDocumentViewer/openFromPhysicalLocation）；`useDocumentContent` hook 获取预签名 URL；`EvidenceReferencePanel` 400px 右滑面板显示证据链+完整性状态；`EvidenceCard` 按 sourceType 分发展示（含 SourceStatusBadge/LocationLink）；`DocumentViewer` 支持 PDF(iframe)/Markdown(react-markdown)/文本(pre) 嵌入查看；`EvidenceChips` 可点击打开面板；`parseEvidenceRefs` 解析 `[ref:evidenceId]` 为可点击上标；`InlineEvidenceRef` 蓝色上标组件；`TimelineIO.TextWithRefs` 在 JSON tree 字符串值中渲染内联引用；已集成到 `ExecutionDebugView`

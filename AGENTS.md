...
- **Story 6-3 已完成**: 执行时间线垂直视图与决策注解。Server: `QueryEvidenceSchema` 新增 `sourceType` 和 `nodeId` 可选过滤参数，`findByExecution()` 支持按 sourceType 直接过滤、按 nodeId 通过 execution_steps 关联查找。Studio: 新增 `features/execution/components/timeline/` 垂直时间线组件族（`ExecutionTimelineVertical` CSS grid + @tanstack/react-virtual 虚拟滚动、`TimelineEntry` 可展开条目、`DecisionAnnotation` agent 决策注解含 AutonomyBadge/ReasoningBlock/AlternativesList/InterventionTag、`OutputLevelBadge` L1-L4、`EvidenceChips` 占位、`FailedNodeError` RFC 7807）。`useTimelineData` hook 聚合 ExecutionStep[] + evidence records 返回 TimelineData[]。`ExecutionDebugView` 中间栏已替换为新垂直时间线

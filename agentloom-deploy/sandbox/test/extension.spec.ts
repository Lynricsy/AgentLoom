...
describe('createAgentLoomExtension', () => {
  ...
  describe('MCP tool registration', () => {
    it('registers tools with mcp_ prefix and correct metadata', () => {
      ...
      expect(registeredTools[0].name).toBe('mcp_search');
      expect(registeredTools[0].label).toBe('search');
      expect(registeredTools[0].description).toBe('Search the web');
      expect(registeredTools[0].parameters).toEqual({
        type: 'object',
    ...
    it('registers multiple tools', () => {
      const factory = createAgentLoomExtension({
        ...
        mcpTools: [
          ...
          { name: 'tool-c', description: 'C', parameters: {} },
        ],
      });
    ...
    it('registered tool execute delegates to provided execute fn', async () => {
      const executeMock = vi.fn().mockResolvedValue({ data: 42 });
      ...
      expect(executeMock).toHaveBeenCalledWith({ x: 10 });
      expect(result.resultForAssistant).toContain('"data": 42');
    });

    it('returns string result directly without JSON.stringify', async () => {
      const factory = createAgentLoomExtension({
        sessionId: 'sess-1',
        mcpTools: [
          {
      ...
      expect(result.resultForAssistant).toBe('hello world');
    });

    it('returns error message when execute throws', async () => {
      const factory = createAgentLoomExtension({
        sessionId: 'sess-1',
        mcpTools: [
          {
      ...
      expect(result.resultForAssistant).toBe('Error: boom');
    });

    it('returns fallback message when no execute fn provided', async () => {
      const factory = createAgentLoomExtension({
        sessionId: 'sess-1',
        mcpTools: [
      ...
      expect(result.resultForAssistant).toBe(
        'MCP tool "noop" has no execute handler',
      );
    });
  });
